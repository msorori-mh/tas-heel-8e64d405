# لا ترسل الخطة بصيغتها الحالية. هي جيدة معماريًا، لكنها تحتاج تعديلات حاكمة قبل أي Production Write.

`FINAL_REVIEW_VERDICT=NEEDS_REVISION_BEFORE_PRODUCTION`

أهم التصحيحات:

1. ممنوع اختلاق `ready_by`

لا تستخدم «فاعلًا نظاميًا» على أنه صاحب اعتماد. استخدم `ready_by` فقط إذا وجد انتقال اعتماد حقيقي مطابق في `audit_logs`.

أما الصفوف القديمة الأخرى، فأضف provenance صريحًا مثل:

```text
evidence_origin=LEGACY_20C_VISIBLE_BASELINE
```

مع snapshot/hash و`ready_at`، دون ادعاء مراجعة بشرية. ويلزم تحديث Preflight/Postverify لقبول هذا النوع الموثق من الإرث. حاليًا Postverify يشترط `ready_by` لكل READY، ولذلك الخطة الحالية ستنتج دليلًا غير صادق أو ستفشل. [postverify-21h.sql](https://github.com/msorori-mh/tas-heel-8e64d405/blob/62c01c801bc04e88106fe5c92e061c47dd8d1f48/scripts/content-v3/postverify-21h.sql)

2. لا تحذف `originalBookPdf` من lifecycle

الأفضل:

```text
status=REVIEW أو DRAFT
applicability=NA بعد 21H
retirement_origin=LEGACY_20C
```

مع الاحتفاظ بالسجل نفسه. النقل إلى archive ثم الحذف يتعارض مع حفظ lifecycle history.

يلزم تعديل Preflight كي لا يعتبر مجرد وجود السجل blocker إذا كان متقاعدًا وغير READY. Migration 21H أصلًا تعرف `originalBookPdf` وتحوّل applicability إلى `NA`. [Migration 21H](https://github.com/msorori-mh/tas-heel-8e64d405/blob/62c01c801bc04e88106fe5c92e061c47dd8d1f48/supabase/migrations-pending/20260818210000_content_v3_21h_hardened_preflight.sql)

3. يجب تعريف snapshot canonical بدقة

لا يكفي قول «من المحتوى الحي». يجب تحديد JSON وترتيبه لكل قدرة، خصوصًا:

- تعدد `lesson_resources`.
- ترتيب الأسئلة والخيارات.
- revisions المنشورة فقط.
- منع الإجابات والتبريرات من snapshot.
- SHA256 على UTF‑8 canonical JSON ثابت.

4. لا تمنح `sandbox_exec` صلاحيات إنتاجية دائمة

خصوصًا لا تمنحه قراءة دائمة على `supabase_migrations`.

نفّذ القياس من خلال:

- Database owner داخل `READ ONLY transaction`، أو
- دور operator مؤقت محدود ثم إلغاء الصلاحيات.

أما `can_access_lesson` فلن يعمل بمجرد `GRANT EXECUTE` لأن `auth.uid()` سيظل فارغًا؛ اختباره يحتاج JWT لطالب اختباري أو قياس predicates مباشرة.

5. ثبّت mapping دورة الحياة

أسماء القدرات الخارجية لا تطابق أسماء lifecycle الحالية حرفيًا:


| الحزمة                   | lifecycle الإنتاجي    |
| ------------------------ | --------------------- |
...
6. لا تستخدم قيمًا افتراضية مجهولة للهوية

إذا لم يثبت `semester` و`sort_order` وملكية المنهج، لا تجعل الدرس READY ولا تدّعي ظهوره النهائي.
...
يجب حل الهوية من المصدر الرسمي أولًا. وضع قيمة افتراضية ثم وصفها `PENDING` قد ينشئ ترتيبًا إنتاجيًا خاطئًا.

7. افصل التنفيذ إلى ثلاث بوابات

الترتيب الصحيح:

```text
R5 SOURCE REMEDIATION
→ PR + PG17 + independent review
...
الخلاصة: اعتماد فكرتك العامة، لكن لا نعتمد ملء `ready_by` المصطنع، ولا حذف lifecycle التاريخي، ولا إنشاء درس بهوية غير محسومة. بعد إدخال هذه التصحيحات تصبح الخطة آمنة للتنفيذ.خطة تحميل حزمة Content V3 إلى الإنتاج (Migration + Content Import)
