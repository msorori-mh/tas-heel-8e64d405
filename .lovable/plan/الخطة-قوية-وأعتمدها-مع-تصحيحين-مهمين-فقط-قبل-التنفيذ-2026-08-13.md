# الخطة قوية، وأعتمدها مع **تصحيحين مهمين فقط** قبل التنفيذ:

```text
SEC-PURGE-GUARD-11B = GO
FIRST_REAL_CONTENT_BATCH_12 = GO_TO_PREVIEW_GATE

```

### 1) الجزء أ — اعتمده، لكن احذف مسار الـfallback المباشر

بعد حذف `qb_e2e_purge_questions` لا أريد أن يتحول:
...
إلى حذف مباشر واسع بـ`service_role` يستطيع تجاوز منطق الـQB أو الاصطدام بتجميد النسخ المنشورة.

الأصح:

```text
shared production DB:
- لا cleanup مباشر لأشجار أسئلة منشورة
- أي teardown عادي يستخدم المسارات الآمنة الموجودة فقط
- إذا واجه Published immutable data → FAIL CLOSED
...
```text
qb_e2e_purge_questions = PERMANENTLY ABSENT

```

ولا نعيد إنشاءها مستقبلاً على القاعدة المشتركة لمجرد E2E. الاختبارات المدمرة المستقبلية للأسئلة المنشورة تكون على PG17 المعزول قدر الإمكان.
...
harden
→ prove anon denied
→ prove authenticated denied
→ prove service_role scope guard
→ verify audit
→ DROP FUNCTION
→ verify pg_proc = 0
...
### 2) أول دفعة حقيقية يجب أن تكون INSERT-only

بما أن الجرد أثبت عدم وجود مادة مكتملة نريد البناء عليها، وأول Batch سيكون **محتوى جديداً**، فأشدد بوابة Preview:

```text
FIRST_REAL_CONTENT_BATCH_12 expected actions:

subjects             INSERT
units                INSERT
lessons              INSERT
lesson contents      INSERT
explanations         INSERT
resources            INSERT
assessments           INSERT
...
نوقف الدفعة قبل Execute ونراجع سبب تطابق الأكواد مع محتوى قائم.

`SKIP` أيضاً لا أتوقعه في أول تشغيل لدفعة جديدة؛ إذا ظهر يجب تفسيره قبل التنفيذ.

### ما أعتمده للدفعة الأولى

```text
Batch Code:
PROD-B12-20260813-01
...
تأتي **حصراً من ملفات المحتوى الحقيقية**، كما ذكرت، ولا نخترعها.

### Snapshot

بالإضافة إلى CSV، أريد أن يحتوي Snapshot قبل التنفيذ على:

```text
baseline row counts
+ exact parent rows
+ proof that proposed natural keys do not exist
+ content_review_state baseline
+ import_jobs baseline
+ timestamp
+ batch code

```

وبما أنها دفعة INSERT-only، يصبح Rollback أكثر أماناً لأننا نستطيع تحديد كل الكيانات الجديدة بأكواد الدفعة دون استعادة قيم قديمة.

### بوابة التنفيذ الفعلية

بعد Validate + Prepare، لا Execute إلا إذا كانت المعاينة بالشكل:

```text
Batch             PROD-B12-20260813-01

subject_code      <real code>
unit_code         <real code>
lesson_code       <real code>
...
Snapshot            VERIFIED
Student visibility  expected DRAFT/HIDDEN
...
student exposure            ZERO
direct-by-id exposure       ZERO
exact replay                ALL SKIPPED
stuck applying jobs         ZERO
unexpected domain changes   ZERO
...
PRODUCTION_IMPORT_OPERATION = PROVEN

```

**إذن الخطة معتمدة.** نفّذوا أولاً `SEC-PURGE-GUARD-11B` بالكامل، وبعد إثبات حذف الدالة ننتقل مباشرة إلى تجهيز ملفات الدفعة 12 حتى Preview؛ أول Execute حقيقي لا يتم إلا بعد أن تثبت المعاينة أنها **INSERT-only وفي النطاق المتوقع**.SEC-PURGE-GUARD-11B + FIRST_REAL_CONTENT_BATCH_12
