...
export function preflightWorkbook(
  ...
  if ((input.fileBytes ?? 0) > DEFAULT_IMPORT_LIMITS.maxFileBytes) {
    issues.push(
      issue(QB_IMPORT_CODES.FILE_TOO_LARGE, {
        file: input.fileName,
        stage: "PREFLIGHT_RAW",
        source_subsystem: "zip-preflight",
      }),
  ...
  if ((meta.zipEntries ?? 0) > DEFAULT_IMPORT_LIMITS.maxZipEntries) {
    issues.push(
      issue(QB_IMPORT_CODES.ZIP_ENTRY_LIMIT, {
        file: input.fileName,
        stage: "PREFLIGHT_ZIP",
        source_subsystem: "zip-preflight",
      }),
  ...
  if ((meta.uncompressedBytes ?? 0) > DEFAULT_IMPORT_LIMITS.maxUncompressedBytes) {
    issues.push(
      issue(QB_IMPORT_CODES.ZIP_TOTAL_SIZE_LIMIT, {
        file: input.fileName,
        stage: "PREFLIGHT_ZIP",
        source_subsystem: "zip-preflight",
      }),
  ...
  if (meta.hasZipBomb) {
    issues.push(
      issue(QB_IMPORT_CODES.ZIP_BOMB_SUSPECTED, {
        file: input.fileName,
        stage: "PREFLIGHT_ZIP",
        source_subsystem: "zip-preflight",
      }),
  ...
  if (meta.hasFormulaCells && !options?.skipFormulaCheck) {
    issues.push(
      issue(QB_IMPORT_CODES.FORMULA_CELL, {
        file: input.fileName,
        stage: "PREFLIGHT_OOXML",
  ...
  if (meta.hasMacros) {
    issues.push(
      issue(QB_IMPORT_CODES.MACRO_CONTENT, {
        file: input.fileName,
        stage: "PREFLIGHT_OOXML",
        source_subsystem: "workbook-parser",
      }),
  ...
  if (meta.hasPathTraversal || meta.suspiciousMediaPaths) {
    issues.push(
      issue(QB_IMPORT_CODES.PATH_TRAVERSAL, {
        file: input.fileName,
        stage: "PREFLIGHT_ZIP",
        source_subsystem: "zip-preflight",
      }),
  ...
  for (const header of input.headers) {
    ...
    if (seen.has(normalized)) {
      issues.push(
        issue(QB_IMPORT_CODES.DUPLICATE_HEADER, {
          file: input.fileName,
          stage: "PREFLIGHT_OOXML",
    ...
    if (/^(id|uuid|role|status|publish|approved_by|publisher|owner_role)$/i.test(normalized)) {
      issues.push(
        issue(QB_IMPORT_CODES.FORBIDDEN_COLUMN, {
          file: input.fileName,
      ...
      issues.push(
        issue(QB_IMPORT_CODES.PRIVILEGE_ESCALATION, {
          file: input.fileName,
          stage: "AUTHORIZATION",
          source_subsystem: "preflight",
        }),
  ...
  for (const row of input.rows) {
    for (const value of Object.values(row)) {
      ...
      if (isFormulaLike(value) || meta.csvInjectionCells) {
        issues.push(
          issue(QB_IMPORT_CODES.FORMULA_INJECTION, {
            file: input.fileName,
            stage: "PREFLIGHT_OOXML",
