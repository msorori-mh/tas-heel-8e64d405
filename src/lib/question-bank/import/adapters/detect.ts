...
export function detectSchemaFromHeaders(headers: string[]): {
  ...
  const same = (expected: readonly string[]) =>
    ...
    expected.every((h, index) => normalizeHeader(h) === actual[index]);
  ...
  if (same(LEGACY_FLAT_HEADERS))
    return { schema: LEGACY_FLAT_15COL, column_shift_suspected: false };

  return {
    schema: "unknown",
    column_shift_suspected: actual.some((h) =>
      [...teacher, ...official, ...LEGACY_FLAT_HEADERS].includes(h),
    ),
  };
}

export const CONTRACT_HEADERS = {
  [TEACHER_FLAT_AR_V0]: teacher,
  [OFFICIAL_FLAT_V0]: official,
  [LEGACY_FLAT_15COL]: LEGACY_FLAT_HEADERS,
} as const;
