...
function isMissingSchema(error: { code?: string; message: string }): boolean {
  return (
    ...
    (/golden_lesson_(packages|stage_manifest)/i.test(error.message) &&
...
export const getGoldenLessonPersistenceCapability = createServerFn({ method: "GET" })
  ...
  .handler(
    async ({
      context,
    }): Promise<{ available: boolean; reason: "AVAILABLE" | "SCHEMA_NOT_APPLIED" }> => {
      ...
      if (result.error && isMissingSchema(result.error))
        return { available: false, reason: "SCHEMA_NOT_APPLIED" };
      if (result.error) throw new Error(result.error.message);
...
export const listGoldenLessonPackages = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<GoldenPackageSummary[]> => {
    ...
    return rows.map((row) => ({
      ...
      reviewStatus: row.review_status as GoldenPersistentReviewStatus,
...
export const getGoldenLessonPackageHistory = createServerFn({ method: "GET" })
  ...
  .handler(
    async ({
      data,
      context,
    }): Promise<{ versions: GoldenPackageVersion[]; reviews: GoldenPackageReview[] }> => {
      const db = pendingClient(context as ContentStaffAuthContext);
      const [versionsResult, reviewsResult] = await Promise.all([
        db
          .from("golden_lesson_package_versions")
          .select("version,client_manifest_sha256,canonical_manifest_sha256,created_at")
          .eq("package_id", data)
          .order("version", { ascending: false }),
        db
          .from("golden_lesson_package_reviews")
          .select("package_version,from_status,to_status,actor_role,evidence,note,created_at")
          .eq("package_id", data)
      ...
      return {
        versions: versions.map((row) => ({
          ...
          clientManifestSha256: String(row.client_manifest_sha256),
          canonicalManifestSha256: String(row.canonical_manifest_sha256),
          createdAt: String(row.created_at),
        })),
        reviews: reviews.map((row) => ({
          packageVersion: Number(row.package_version),
          fromStatus: row.from_status as GoldenPersistentReviewStatus,
          toStatus: row.to_status as GoldenPersistentReviewStatus,
          actorRole: row.actor_role as GoldenPackageReview["actorRole"],
          evidence: (row.evidence ?? {}) as Record<string, boolean | string | number | null>,
...
export const stageGoldenLessonManifest = createServerFn({ method: "POST" })
  ...
  .handler(
    async ({
      ...
      packageId: string;
      version: number;
      status: GoldenPersistentReviewStatus;
      idempotent: boolean;
      writesPerformed: number;
    }> => {
      const manifest = parseGoldenLessonManifest(JSON.stringify(data.manifest));
      if (!previewGoldenLessonStaging(manifest).valid)
        throw new Error("MANIFEST_SERVER_VALIDATION_FAILED");
      const result = assertDb(
        await pendingClient(context as ContentStaffAuthContext).rpc(
          "golden_lesson_stage_manifest",
          {
            _manifest: manifest as GoldenLessonPackage,
            _client_manifest_sha256: data.clientManifestSha256,
          },
        ),
