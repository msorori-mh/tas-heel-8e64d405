...
export const stageApprovedGoldenLessonDomainBundle = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const packageResult = await admin.from("golden_lesson_packages").select("current_version,review_status")
      .eq("id", data.packageId).single();
    if (packageResult.error || !packageResult.data) throw new Error(packageResult.error?.message ?? "PACKAGE_NOT_FOUND");
    if (packageResult.data.current_version !== data.version || packageResult.data.review_status !== "APPROVED_FOR_STAGING") {
      throw new Error("PACKAGE_NOT_APPROVED_FOR_DOMAIN_STAGING");
    }
    const versionResult = await admin.from("golden_lesson_package_versions")
      .select("verified_storage_path,verified_bundle_sha256,bundle_verified_at")
      .eq("package_id", data.packageId).eq("version", data.version).single();
    if (versionResult.error || !versionResult.data?.verified_storage_path || !versionResult.data.verified_bundle_sha256 || !versionResult.data.bundle_verified_at) {
      throw new Error(versionResult.error?.message ?? "VERIFIED_BUNDLE_REQUIRED");
    }
    const downloaded = await admin.storage.from(BUCKET).download(versionResult.data.verified_storage_path);
    ...
    if (verified.bundleSha256 !== versionResult.data.verified_bundle_sha256) throw new Error("VERIFIED_BUNDLE_IDENTITY_MISMATCH");
    const envelope = buildGoldenDomainStageEnvelope(verified);
    const staged = await admin.rpc("golden_lesson_stage_domain_bundle" as never, {
    ...
    if (staged.error || !staged.data) throw new Error(staged.error?.message ?? "DOMAIN_STAGE_EMPTY_RESPONSE");
