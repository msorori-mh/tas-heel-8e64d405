...
export const publishGoldenLessonDirect = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ steps: DirectPublishStep[]; lessonId: string; batchId: string }> => {
    ...
    const rpc = ((name, args) =>
      (supabase as unknown as { rpc: Rpc }).rpc(name, args)) as Rpc;
    ...
    if (status === "DRAFT") {
      record(await rpc("golden_lesson_advance_review", {
    ...
    if (status !== "APPROVED_FOR_STAGING") {
      const approved = record(await rpc("golden_lesson_owner_approve_for_staging", {
        _package_id: data.packageId,
        _expected_version: data.version,
        _evidence: {
        ...
        _reason: "اعتماد المالك المباشر بعد اكتمال فحوصات الملفات السبعة والتحقق الخادمي من البصمات.",
    ...
    const { buildDirectDomainStageEnvelope } = await import("./golden-lesson-direct-publish.server");
    const { envelope, bundleSha256 } = await buildDirectDomainStageEnvelope(data.packageId, data.version);
    const { createClient } = await import("@supabase/supabase-js");
    const admin = createClient(process.env['SUPABASE_URL']!, process.env['SUPABASE_SERVICE_ROLE_KEY']!, {
      auth: { persistSession: false, autoRefreshToken: false },
    });
    const staged = record(await (admin as unknown as { rpc: Rpc }).rpc("golden_lesson_stage_domain_bundle", {
      _package_id: data.packageId,
    ...
    const bound = record(await rpc("golden_lesson_bind_authoritative_identity_operator", {
      _batch_id: batchId,
      _actor_id: userId,
    ...
    const {
      asRpcResult, attestStoredAssets, ensureVerifiedAssets, idempotencyKey, planSha,
    } = await import("./golden-lesson-publication.server");

    const dryMaterialize = record(await rpc("golden_lesson_materialize_domain_batch_operator", {
      _batch_id: batchId, _actor_id: userId, _mode: "DRY_RUN",
      _expected_plan_sha256: null, _idempotency_key: null,
    }), "CF10_DRY_RUN_FAILED");
    ...
    record(await rpc("golden_lesson_materialize_domain_batch_operator", {
    ...
    await attestStoredAssets(userId, batchId, ensured.declarations, ensured.uploadedPaths, "EXECUTE");
    push("assets", "التحقق من المرفقات", `${ensured.declarations.length} ملف`);

    const dryPublish = record(await rpc("golden_lesson_publish_cf11", {
      _batch_id: batchId, _actor_id: userId, _mode: "DRY_RUN",
    ...
    const published = record(await rpc("golden_lesson_publish_cf11", {
      _batch_id: batchId, _actor_id: userId, _mode: "EXECUTE",
      _assets: ensured.declarations, _expected_plan_sha256: publishPlan,
      _idempotency_key: idempotencyKey("cf11", batchId, publishPlan),
    }), "CF11_PUBLISH_FAILED");
    asRpcResult(published);
    push("publish", "نشر الدرس", "تم");

    record(await rpc("golden_lesson_attest_cf11_ready", {
      _batch_id: batchId, _actor_id: userId, _mode: "EXECUTE",
      _evidence: {
