...
export function validateGoldenLessonPackage(pkg: GoldenLessonPackage): GoldenLessonValidationResult {
  ...
  const schemaSupported = isGoldenLessonSchema(pkg.schema);
  if (!schemaSupported) error("SCHEMA_UNSUPPORTED", "schema", "إصدار مخطط الحزمة غير مدعوم.");
  const expectedCapabilities: readonly GoldenCapability[] = schemaSupported
    ? capabilitiesForGoldenLessonSchema(pkg.schema)
    : [];
  const expectedSet = new Set<GoldenCapability>(expectedCapabilities);
  const profile = getGoldenLessonProfile(pkg.profileId);
  if (!profile) error("PROFILE_UNKNOWN", "profileId", "نمط الدرس الذهبي غير معروف.");
  if (profile && profile.schema !== pkg.schema) {
    error("PROFILE_SCHEMA_MISMATCH", "profileId", "إصدار النمط لا يطابق إصدار مخطط الحزمة.");
  }
  ...
  if (schemaSupported && !sameOrder(pkg.capabilityOrder, expectedCapabilities)) {
  ...
  for (const artifact of pkg.artifacts) {
    if (!expectedSet.has(artifact.capability)) {
      error("CAPABILITY_UNSUPPORTED_FOR_SCHEMA", `artifacts.${String(artifact.capability)}`, "القدرة لا تنتمي إلى إصدار الحزمة المختار.");
      continue;
    }
    ...
    const profileApplicability = profile?.applicability[artifact.capability];
    const applicabilityMatches = profileApplicability === artifact.applicability ||
      (profileApplicability === "OPTIONAL" && artifact.applicability === "NA") ||
      (artifact.capability === "equationsAndLawsHtml" &&
        profileApplicability === "REQUIRED" && artifact.applicability === "NA");
    if (profile && (!profileApplicability || !applicabilityMatches)) {
    ...
    if (artifact.sourcePath) {
      for (const finding of validateGoldenLessonArtifactPath(artifact.capability, artifact.sourcePath, pkg.schema).findings) {
