...
const PATTERNS: Array<{ test: RegExp; message: string }> = [
  {
    ...
    message: "بيانات الدخول غير صحيحة. تأكد من البريد/الهاتف وكلمة المرور.",
  },
  {
    test: /email not confirmed/i,
    message: "لم يتم تأكيد بريدك الإلكتروني بعد. يرجى فتح رسالة التأكيد.",
  },
  {
    test: /user already registered|already.*registered|user.*exists/i,
    message: "هذا الحساب مسجّل مسبقًا. سجّل دخولك بدلًا من إنشاء حساب جديد.",
  },
  {
    test: /password should be at least.*(\d+).*characters?/i,
    message: "كلمة المرور قصيرة جدًا. استخدم 6 أحرف على الأقل.",
  },
  {
    test: /password.*(at least|minimum)/i,
    message: "كلمة المرور قصيرة جدًا. استخدم 6 أحرف على الأقل.",
  },
  {
    test: /signup requires a valid password/i,
  ...
  {
    test: /phone.*rate limit|sms.*rate limit/i,
    message: "تم إرسال عدة رموز خلال وقت قصير. حاول مرة أخرى بعد قليل.",
  },
  {
    test: /email rate limit/i,
    message: "تم إرسال عدة رسائل خلال وقت قصير. حاول مرة أخرى بعد قليل.",
  },
  { test: /rate limit/i, message: "محاولات كثيرة خلال وقت قصير. انتظر قليلًا ثم حاول من جديد." },
  {
    test: /token has expired or is invalid|otp.*expired|expired.*otp/i,
  ...
  },
  {
    ...
    message: "تعذّر الاتصال بالخادم. تحقق من الإنترنت ثم حاول مجددًا.",
  },
];

export function translateAuthError(err: unknown): string {
  const candidate = err && typeof err === "object" ? (err as Record<string, unknown>) : null;
  const raw =
