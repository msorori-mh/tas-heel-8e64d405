...
export function GoldenLessonManifestReviewPanel() {
  ...
  const [persistence, setPersistence] = useState<{ available: boolean; reason: string }>({
    available: false,
    reason: "CHECKING",
  });
  ...
  const selectPackage = useCallback(
    async (pkg: GoldenPackageSummary) => {
      setSelected(pkg);
      setBusy(true);
      try {
        ...
        setMessage(error instanceof Error ? error.message : "تعذر تحميل السجل.");
      } finally {
        setBusy(false);
      }
    },
  ...
  const next = useMemo(
    () =>
      selected
        ? GOLDEN_REVIEW_TRANSITIONS.find((item) => item.from === selected.reviewStatus)
        : undefined,
    [selected],
  );
  const loadManifest = async (file?: File) => {
    ...
    try {
      ...
      const parsed = parseGoldenLessonManifest(raw);
      const result = previewGoldenLessonStaging(parsed);
      setManifest(parsed as GoldenLessonPackage);
      setPreview(result);
      setManifestHash(await sha256Text(raw));
      setEvidence((current) => ({ ...current, packageValidationPassed: result.valid }));
      setMessage(
        result.valid
          ? "نجح Dry-Run المحلي. الحفظ في staging متاح فقط بعد تطبيق المخطط."
          : "فشل Dry-Run: صحح الأخطاء.",
      );
    } catch (error) {
      setManifest(null);
      setPreview(null);
  ...
  const persistManifest = async () => {
    ...
    try {
      ...
      setMessage(
        `حُفظت بيانات staging: الإصدار ${result.version}، الكتابات ${result.writesPerformed}، كتابات المحتوى 0.`,
      );
      await refresh();
      const current = (await loadPackages()).find(
        (pkg: GoldenPackageSummary) => pkg.id === result.packageId,
      );
      if (current) await selectPackage(current);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "تعذر حفظ staging.");
    } finally {
      setBusy(false);
    }
  };
    ...
    try {
      const result = await advanceReview({
        data: {
          ...
          expectedVersion: selectedPackage.currentVersion,
          toStatus: next.to,
          evidence,
          note: null,
        },
      ...
      if (result.status === "APPROVED_FOR_STAGING") {
        const staged = await stageDomain({
          data: { packageId: selectedPackage.id, version: selectedPackage.currentVersion },
        });
        await bindIdentity({ data: { batchId: staged.batchId } });
        automaticPreparation = " وتم تجهيزها وربطها بالدرس تلقائيًا.";
      }
      setMessage(
        `تم تحديث حالة المراجعة إلى ${STATUS_LABEL[result.status as GoldenReviewStatus]}${automaticPreparation}`,
      );
      const latest = await loadPackages();
      setPackages(latest);
      const current = latest.find((pkg: GoldenPackageSummary) => pkg.id === selectedPackage.id);
      if (current) await selectPackage(current);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "رفض الخادم الانتقال.");
    } finally {
      setBusy(false);
    }
  };
  const persistOwnerApproval = async () => {
    ...
    setMessage(null);
    try {
      const result = await ownerApprove({
        data: {
      ...
      setMessage(
        `تم اعتماد مالك المنصة للتجهيز مع سجل تدقيق. كتابات المحتوى: ${result.domainWritesPerformed}.`,
      );
      const latest = await loadPackages();
      setPackages(latest);
      const current = latest.find((pkg: GoldenPackageSummary) => pkg.id === selected.id);
      if (current) await selectPackage(current);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "رفض الخادم اعتماد مالك المنصة.");
    } finally {
      setBusy(false);
    }
  };
  const stageAndBindSelectedPackage = async () => {
    ...
    try {
      const staged = await stageDomain({
        data: { packageId: selected.id, version: selected.currentVersion },
      });
      const bound = await bindIdentity({ data: { batchId: staged.batchId } });
      setMessage(
        `تم تجهيز الحزمة وربط هوية الدرس. batch=${staged.batchId.slice(0, 8)}… · binding=${bound.bindingId.slice(0, 8)}… · كتابات المحتوى 0. حدّث لوحة CF11 أدناه.`,
      );
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "تعذر تجهيز الحزمة وربط الدرس.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <section
      dir="rtl"
      aria-labelledby="golden-review-heading"
      className="rounded-2xl border border-primary/25 bg-card p-5 shadow-card space-y-5"
    >
      <div className="flex flex-wrap items-center gap-2">
        <ClipboardCheck className="h-5 w-5 text-primary" />
        <div className="min-w-0 flex-1">
          <h2 id="golden-review-heading" className="text-lg font-semibold">
            مسودات الدروس الواردة للمراجعة
          </h2>
          <p className="mt-1 text-xs text-muted-foreground">
            اختر المسودة، تحقق من الأدلة، ثم اطلب التعديل أو اعتمدها. التجهيز التقني يتم تلقائيًا
            بعد الاعتماد.
          </p>
        </div>
        <Button
      ...
      {!persistence.available && (
        <p className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-3 text-sm">
          خدمة المراجعة غير متاحة حاليًا؛ أوقِف الاعتماد حتى عودتها.
        </p>
      )}
      {busy && (
      ...
      {persistence.available && (
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="rounded-xl border p-4 space-y-2">
            <h3 className="font-medium">المسودات الواردة</h3>
            {packages.length === 0 && (
              <p className="text-sm text-muted-foreground">لا توجد حزم محفوظة.</p>
            )}
            {packages.map((pkg) => (
              <button
                key={pkg.id}
                ...
                className="w-full min-h-[44px] rounded-lg border p-3 text-start hover:bg-muted/50"
              >
                <span className="font-mono text-xs">{pkg.packageCode}</span>
                <span className="float-left">
                  <Badge>{STATUS_LABEL[pkg.reviewStatus]}</Badge>{" "}
                  <Badge variant="outline">v{pkg.currentVersion}</Badge>
                </span>
              </button>
            ))}
          </div>
          <div className="rounded-xl border p-4 space-y-3">
            <h3 className="font-medium">سجل الإصدارات والمراجعة</h3>
            {selected ? (
              <>
                <p className="font-mono text-xs">{selected.packageCode}</p>
                <div className="max-h-48 overflow-auto space-y-1">
                  {versions.map((version) => (
                    <p key={version.version} className="rounded border p-2 text-xs">
                      الإصدار {version.version}
                    </p>
                  ))}
                  {reviews.map((review, index) => (
                    <p key={`${review.createdAt}-${index}`} className="rounded border p-2 text-xs">
                      v{review.packageVersion}: {review.fromStatus} → {review.toStatus} ·{" "}
                      {review.actorRole}
                    </p>
                  ))}
                </div>
              ...
              <p className="text-sm text-muted-foreground">اختر حزمة.</p>
            )}
          </div>
        </div>
      )}
      ...
      <div className="rounded-xl border p-4 space-y-3">
        <div className="flex items-center justify-between gap-2">
          <span className="font-medium">قرار المراجعة</span>
          <Badge>{selected ? STATUS_LABEL[selected.reviewStatus] : "اختر حزمة"}</Badge>
        </div>
          ...
          {(Object.keys(EVIDENCE_LABEL) as (keyof GoldenReviewEvidence)[]).map((key) => (
            ...
            >
              <input
                ...
                disabled={key === "packageValidationPassed"}
                onChange={(event) =>
                  setEvidence((current) => ({ ...current, [key]: event.target.checked }))
                }
              />
              {EVIDENCE_LABEL[key]}
            </label>
        ...
        <Button
          type="button"
          onClick={() => void persistTransition()}
          disabled={busy || !persistence.available || !selected || !next}
          className="min-h-[44px] gap-2"
        >
          {next ? (
            <>
              <CheckCircle2 className="h-4 w-4" />
              الانتقال إلى {STATUS_LABEL[next.to]}
            </>
          ) : (
            <>
      ...
      {message && (
        <p
          role="status"
          className="text-sm rounded-lg border bg-muted/30 px-3 py-2 flex items-start gap-2"
        >
