...
const TEMPLATES = [
  { prefix: "01", key: "subjects", file: "01_subjects_template.xlsx", label: "المواد", required: ["subject_code", "name", "grade_slug"], codeColumn: "subject_code" },
  { prefix: "02", key: "units", file: "02_units_template.xlsx", label: "الوحدات", required: ["unit_code", "subject_code", "title"], codeColumn: "unit_code" },
  { prefix: "03", key: "lessons", file: "03_lessons_template.xlsx", label: "الدروس", required: ["lesson_code", "subject_code", "title"], codeColumn: "lesson_code" },
  { prefix: "04", key: "book_contents", file: "04_lesson_book_contents_template.xlsx", label: "محتوى الكتاب", required: ["lesson_code", "content"] },
  { prefix: "05", key: "explanations", file: "05_lesson_explanations_template.xlsx", label: "الشروحات", required: ["lesson_code", "title", "content"] },
...
export async function validateContentPackage(dir) {
  ...
  try {
    ...
    issue(errors, String(dir), null, "DIR_NOT_FOUND", `المجلد غير موجود أو غير قابل للقراءة: ${dir}`);
  ...
  for (const file of xlsxFiles) {
    ...
    if (!template) {
      issue(warnings, file.name, null, "EXTRA_FILE", `ملف غير متوقع لا يتبع تسمية القوالب 01–09: «${file.name}».`);
      continue;
    }
    if (filesByTemplate.has(template.key)) {
      issue(errors, file.name, null, "DUPLICATE_TEMPLATE_FILE", `أكثر من ملف للقالب ${template.prefix} (${template.label}).`);
      continue;
    }
  ...
  for (const t of TEMPLATES.filter((t) => !filesByTemplate.has(t.key))) {
    issue(errors, t.file, null, "FILE_MISSING", `الملف المطلوب «${t.file}» (${t.label}) غير موجود في المجلد.`);
  }
  ...
  for (const t of TEMPLATES) {
    ...
    for (const col of t.required) {
      if (!p.headers.includes(col)) {
        issue(errors, fileName, null, "MISSING_COLUMN", `العمود المطلوب «${col}» غير موجود في «${fileName}».`);
      }
    }
    if (p.rows.length === 0) {
    ...
    for (const row of p.rows) {
      for (const col of t.required) {
        if (!row.data[col]?.trim()) {
          issue(errors, fileName, row.rowNumber, "MISSING_VALUE", `الحقل «${col}» مطلوب ولا يمكن أن يكون فارغاً (صف ${row.rowNumber}).`);
        }
      }
      if (t.codeColumn) {
        ...
        if (code) {
          if (set.has(code)) {
            issue(errors, fileName, row.rowNumber, "DUPLICATE_CODE", `تكرار ${t.codeColumn} «${code}» — كل كود يجب أن يكون فريداً.`);
          }
          set.add(code);
        }
      }
  ...
  for (const row of subjectRows) {
    ...
    if (NONSTANDARD_DASH.test(name)) {
      issue(warnings, file, row.rowNumber, "NONSTANDARD_SEPARATOR", `الفاصل في «${name}» غير موحد. المعتمد: " - " (مسافة + شرطة + مسافة).`);
    }
    const parent = getSubjectMainCategory(name);
    if (parent === "الإسلامية") {
      issue(warnings, file, row.rowNumber, "NONSTANDARD_PARENT_SPELLING", `المعتمد دائماً «التربية الإسلامية - اسم القسم» وليس «الإسلامية - ...»: راجع «${name}».`);
    }
    const gradeSlug = row.data.grade_slug?.trim();
    if (gradeSlug && !/^grade-\d+$/.test(gradeSlug)) {
      issue(warnings, file, row.rowNumber, "UNKNOWN_GRADE_SLUG", `grade_slug «${gradeSlug}» غير مألوف — القيم المعروفة: grade-10 | grade-11 | grade-12.`);
    }
    ...
    for (let i = 0; i < parents.length; i++) {
      for (let j = i + 1; j < parents.length; j++) {
        ...
        if (a !== b && (a.includes(b) || b.includes(a))) {
          issue(warnings, file, parentFirstRow.get(b), "PARENT_SPELLING_MISMATCH", `هجاءان مختلفان لنفس المادة الكبرى: «${a}» و«${b}» — وحّد الاسم الكبير حرفياً.`);
        }
      }
    }
  }
  ...
  for (const row of getRows("resources")) {
    ...
    if (type && !RESOURCE_TYPES.includes(type)) {
      issue(errors, filesByTemplate.get("resources"), row.rowNumber, "INVALID_RESOURCE_TYPE", `resource_type «${row.data.resource_type}» غير مدعوم. القيم: ${RESOURCE_TYPES.join(" | ")}.`);
    }
  }
  ...
  const link = (fromKey, fromColumn, toKey, toLabel) => {
    ...
    if (targets.size === 0) {
      if (getRows(fromKey).some((r) => r.data[fromColumn]?.trim())) {
        issue(warnings, fromFile, null, "LINK_SKIPPED", `تعذّر فحص ربط «${fromColumn}» لغياب/فراغ قالب ${toLabel}.`);
      }
      return;
    }
    for (const row of getRows(fromKey)) {
      ...
      if (ref && !targets.has(ref)) {
        issue(errors, fromFile, row.rowNumber, "UNKNOWN_REFERENCE", `«${fromColumn}» = «${row.data[fromColumn]}» غير معرّف في قالب ${toLabel}.`);
      }
    }
  };
...
function printReport(dir, report) {
  ...
  for (const w of report.warnings) {
    console.log(`  ⚠️  [${w.code}] ${w.file}${w.rowNumber ? " صف " + w.rowNumber : ""}: ${w.message}`);
  }
  for (const e of report.errors) {
    console.log(`  ❌ [${e.code}] ${e.file}${e.rowNumber ? " صف " + e.rowNumber : ""}: ${e.message}`);
  }
  console.log(
    report.ok
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (!dir) {
    console.error("الاستخدام: node scripts/content-import/validate-content-package.mjs <content-dir>");
    process.exit(2);
  }
