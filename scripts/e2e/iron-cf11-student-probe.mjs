...
const ANSWER_LEAK =
  /\b(is_correct|correct_index|correct_answer|rationale_ar|"isCorrect"|"correctIndex")\b/;
...
try {
  ...
  console.error(
    "FAIL probe.dependency — playwright is not installed. Run: bun add -d playwright && bunx playwright install chromium",
  );
  process.exit(2);
}
...
try {
  for (const viewport of VIEWPORTS) {
    ...
    const response = await page.goto(`${BASE}/lessons/${LESSON}`, {
    ...
    check(
      `${tag} no subscription gate`,
      !/اشترك الآن|هذا الدرس متاح للمشتركين|paywall/i.test(body),
    );
    ...
    for (const section of SECTION_ORDER) {
      const locator = page
        .locator(`[data-capability="${section}"], #${section}, [data-section="${section}"]`)
        .first();
      if (await locator.count()) present.push(section);
    }
    check(
      `${tag} seven sections present`,
      present.length === SECTION_ORDER.length,
      present.join(","),
    );
    check(
      `${tag} section order preserved`,
      present.join(",") === SECTION_ORDER.slice(0, present.length).join(","),
    );
    ...
    check(
      `${tag} equations keep sub/sup`,
      (await page.locator(".equation sub, .equation sup").count()) > 0,
    );
    ...
    if (await figure.count()) {
      ...
      check(
        `${tag} furnace src is a resolver URL, not a bare leaf`,
        Boolean(src) && !/^official-figure/.test(src ?? ""),
        src ?? "",
      );
    ...
    const mindMap = page
      .locator('iframe[title*="خريطة"], iframe[data-capability="mindMap"]')
      .first();
    if (await mindMap.count()) {
      const sandbox = await mindMap.getAttribute("sandbox");
      check(
        `${tag} mind map iframe allows scripts without same-origin`,
        /allow-scripts/.test(sandbox ?? "") && !/allow-same-origin/.test(sandbox ?? ""),
        sandbox ?? "",
      );
      ...
      if (frame) {
        ...
        check(
          `${tag} mind map uses details/summary`,
          /<details/i.test(html) && /<summary/i.test(html),
        );
      } else {
    ...
    const lab = page
      .locator('iframe[title*="تجربة"], iframe[data-capability="labExperiment"]')
      .first();
    if (await lab.count()) {
      const sandbox = await lab.getAttribute("sandbox");
      check(
        `${tag} lab iframe is sandboxed without same-origin`,
        Boolean(sandbox) && !/allow-same-origin/.test(sandbox ?? ""),
        sandbox ?? "",
      );
      ...
      if (frame) {
        ...
        check(
          `${tag} lab has no external URL`,
          !/https?:\/\//i.test(labHtml.replace(/https?:\/\/www\.w3\.org[^"']*/g, "")),
        );
        check(
          `${tag} lab exposes Fe2+/Fe3+ and reset`,
          /Fe<sup>2\+/.test(labHtml) &&
            /Fe<sup>3\+/.test(labHtml) &&
            /(إعادة|reset)/i.test(labHtml),
        );
      } else {
      ...
      check(
        `${tag} lab experiment present (OPTIONAL capability)`,
        true,
        "absent — allowed only when lab is not READY",
      );
    }
    ...
    check(
      `${tag} five official questions`,
      (await official.count()) === 5,
      String(await official.count()),
    );
    check(
      `${tag} forty self-test questions`,
      (await selfTest.count()) === 40,
      String(await selfTest.count()),
    );
    check(
      `${tag} no revealed answer before checking`,
      (await page.locator("[data-answer-revealed='true']").count()) === 0,
    );
    check(
      `${tag} no rationale before checking`,
      (await page.locator("[data-rationale]:visible").count()) === 0,
    );
    ...
    check(
      `${tag} answer leak = 0 in hydration payload`,
      !scriptPayloads.some((text) => ANSWER_LEAK.test(text)),
    );
    ...
    if (await firstSelf.count()) {
      ...
      if (await option.count()) {
        ...
        if (await checkButton.count()) {
          ...
          await firstSelf
            .locator("[data-rationale]")
            .first()
            .waitFor({ state: "visible", timeout: 5000 })
            .catch(() => {});
          check(
            `${tag} rationale appears only after check`,
            (await firstSelf.locator("[data-rationale]").count()) > 0,
          );
        } else {
    ...
    check(
      `${tag} zero external network requests`,
      external.length === 0,
