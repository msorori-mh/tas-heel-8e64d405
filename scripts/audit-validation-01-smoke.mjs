...
async function testC04() {
  ...
  let publicBody;
  try { publicBody = JSON.parse(publicText); } catch { publicBody = publicText.slice(0, 80); }
  record(
    "C-04a",
    publicRes.status === 200 && Array.isArray(publicBody) && publicBody.length > 0 ? "Confirmed" : publicRes.status === 404 || publicRes.status === 406 ? "False Positive" : "Partial",
  ...
  let extBody;
  try { extBody = JSON.parse(extText); } catch { extBody = extText.slice(0, 80); }
  record(
    "C-04b",
  ...
  record(
    "C-04c",
    rpcAnon.error?.message?.includes("forbidden") || rpcAnon.error?.code === "42501" ? "Partial" : rpcAnon.data ? "Confirmed" : "Partial",
...
async function testC03(client) {
  ...
  record("C-03", leaked ? "Confirmed" : error ? "Partial" : "False Positive", leaked ? "email returned for random UUID [redacted]" : error ? `error: ${error.message}` : `return: ${data}`);
}

async function testBonus(client, ownUserId) {
  const foreign = randomUUID();
  const { data, error } = await client.rpc("has_active_subscription", { _user_id: foreign });
  record("H-03-bonus", error ? "Partial" : typeof data === "boolean" ? "Confirmed" : "Partial", error ? `error: ${error.message}` : `boolean=${data}`);
...
async function testC02(client, userId) {
  ...
  const { data: templates, error: tplErr } = await client.from("exam_templates").select("id").eq("is_active", true).limit(1);
  if (tplErr || !templates?.length) {
    record("C-02", "Partial", `no template: ${tplErr?.message ?? "empty"}`);
    return;
  }
  const templateId = templates[0].id;
  const { data: sessionId, error } = await client.rpc("start_exam_session", { _template_id: templateId });
  record("C-02", sessionId && !error ? "Confirmed" : "Partial", `has_active_subscription=${subFlag}; ${error ? `error: ${error.message}` : `session=${String(sessionId).slice(0,8)}`}`);
}

async function testC01(client, userId) {
  const { data: plans } = await client.from("subscription_plans").select("id,price,currency").eq("is_active", true).limit(1);
  const { data: methods } = await client.from("payment_methods").select("id").eq("is_active", true).limit(1);
  if (!plans?.length || !methods?.length) {
    record("C-01", "Partial", "no plan/method");
    return null;
  }
  const plan = plans[0];
  const { data: inserted, error: insErr } = await client.from("payment_requests").insert({
    user_id: userId,
    plan_id: plan.id,
  ...
  record("C-01-insert", inserted.subscription_id == null ? "Confirmed" : "False Positive", `subscription_id=${inserted.subscription_id ?? "null"}`);
  const { error: apprErr } = await client.rpc("approve_payment_request", { _request_id: inserted.id, _admin_notes: "audit" });
  record("C-01-approve", apprErr?.message?.includes("missing subscription_id") ? "Confirmed" : apprErr?.message?.includes("forbidden") || apprErr?.code === "42501" ? "Partial" : apprErr ? "Partial" : "False Positive", apprErr ? apprErr.message : "success unexpected");
  return inserted.id;
}
