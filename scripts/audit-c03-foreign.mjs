...
const tag = Date.now();
const up = await c1.auth.signUp({ email: `audit-val03+${tag}@example.com`, password: `AuditVal01!${tag}` });
...
console.log("fresh_client_victim", r1.error ? r1.error.message : (r1.data ? `leaked_len_${r1.data.length}` : "null"));
console.log("fresh_client_random", r2.error ? r2.error.message : (r2.data === null ? "null" : `leaked_len_${String(r2.data).length}`));
