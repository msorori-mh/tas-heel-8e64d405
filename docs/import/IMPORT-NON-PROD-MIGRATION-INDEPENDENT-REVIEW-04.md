...
## 3. الملاحظات المانعة للتطبيق (Blocking)

**H-1 — تبعية غير مطبَّقة (يجب إغلاقها قبل التطبيق).**
...
الأثر: DDL ينجح، ثم ينكسر قالب 06 عند أول تنفيذ حقيقي (سيناريو A4).
المعالجة: تطبيق ترحيلَي `content_html` أولاً، ثم ترحيل المرحلة 03 — بهذا الترتيب حصراً.

**H-2 — `lessons` بلا فهرس فريد على `(subject_id, slug)`** في قاعدة البيانات المُدارة.
بدونه لا يوجد ضمان قاعدي لتفرّد الدرس داخل المادة، ويبقى منع التكرار على مستوى التطبيق فقط.
المعالجة: إضافة الفهرس الفريد ضمن ترحيل تمهيدي (بعد فحص التكرارات القائمة).
