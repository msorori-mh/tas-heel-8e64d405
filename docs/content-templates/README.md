...
## 2أ. تسمية المواد المقسّمة (نموذج 01) — إلزامي للصف الأول
...
- الفاصل: **مسافة + شرطة + مسافة** (` - `) حرفياً. الشرطات من نوع `– — − ‐ ―` تُظهر تحذيراً في dry-run — حوّلها إلى `-`.
- المادة بدون فاصل تظهر مادة عادية (مثال: `الفيزياء`).
- ⚠️ **المعتمد دائماً `التربية الإسلامية - ...`** — لا تستخدم `الإسلامية - ...` (يظهر تحذير في dry-run).
- وحّد هجاء اسم المادة الكبرى عبر كل أقسامها حرفياً — أي اختلاف يعرضها كمواد منفصلة للطالب (يظهر تحذير `PARENT_SPELLING_MISMATCH`).
- القيم الثماني المعتمدة للصف الأول (sort_order/color/icon): `docs/SUBJECT-GROUPING-GRADE-10-YEMEN-CONTENT-GUIDE.md`.
