...
| --- | --- | --- | --- |
| CRITICAL | anon / lesson RPC | The latest local `can_access_lesson` replacement omits the explicit `auth.uid() IS NOT NULL` guard. Earlier SQL revokes EXECUTE from `anon` but does not revoke the default function privilege from `PUBLIC`; membership in `PUBLIC` can therefore keep the RPC executable. `user_can_access_subject_curriculum` can return true for a subject with a null curriculum track even when there is no profile. | User-approved hardening migration plus verification against the deployed DB grants. Explicitly revoke from `PUBLIC` and `anon`, grant only to `authenticated`, and retain an in-function auth guard. |
...
| HIGH | subject-only questions | `can_access_subject` remains subscription-gated in the baseline migration and is not replaced by the free-access migration. Questions with `lesson_id IS NULL` therefore do not consistently follow the free-access pivot. | User-approved migration replacing `can_access_subject` with authenticated + grade + curriculum checks and no subscription requirement. |
| HIGH | staff route separation | The authenticated layout exempts `content_manager` from profile completion but does not redirect it away from student routes (`/app`, lessons, exams, wallet, subscription, payments). Admin-section helpers correctly deny full-admin financial paths, but that does not prevent direct student-route entry. | A separate code PR should add a centralized staff/student route boundary. Coordinate ownership of `src/routes/_authenticated/route.tsx`; do not solve this only with hidden navigation. |
...
Security review count: **CRITICAL=1 / HIGH=3 / MEDIUM=0**. The required acceptance
...
| anon denied | Explicit in-function denial exists for exam/practice; HOLD for lesson/subject helper privilege chain. |
...
The static suite covers the current free-access RPC contract, detects the three
database gaps above, checks financial UI freezing, admin-finance separation,
