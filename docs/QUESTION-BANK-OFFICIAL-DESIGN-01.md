...
1. إضافة البنية الجديدة (فارغة) + RLS صارم
2. Backfill من JSON/correct_index/explanation/lesson/subject
3. تشغيل طبقة التوافق (sync → Legacy)
4. تحويل الاستيراد للكتابة المطبّعة + sync
5. تحويل لوحة الإدارة
6. تحويل واجهات الطالب/التصحيح تدريجياً
7. إثبات صفر قراءة SoT من Legacy
8. Migration منفصلة لحذف Legacy (مستقبلاً فقط)
