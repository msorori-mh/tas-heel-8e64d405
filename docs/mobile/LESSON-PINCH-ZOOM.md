...
Opaque lesson iframes do not bubble touch events to the parent React component. Android now observes the WebView's native touch stream and hit-tests the parent document on the first finger. Two fingers inside a lesson viewport scale that component between 100% and 300%. Single-finger taps and scrolling remain WebView-owned. Native cancellation ends the initial touch when pinch takes ownership; pointer-count changes rebase distance.
