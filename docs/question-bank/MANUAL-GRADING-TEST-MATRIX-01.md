...
| **TC-AFR-012** | الإفراج المؤجل لنتائج الممارسة التدريبية (Practice DELAYED Release) | `system` / `scheduler` / `worker` | نشاط بموجه DELAYED، وإجابة معتمدة مسبقاً (`FINALIZED`)، وزمن `release_at` / `reveal_at` محفوظ بـ UTC، وعدم وجود حاجة لاكتمال الدفعة | تشغيل Worker/Scheduler عند انقضاء الوقت بـ UTC وتطبيق not-before-time guard وconfig validation | الإفراج بـ UTC وتحويل Timezone عند العرض فقط، وضمان Idempotency وRetry وFailure recovery وStale job handling وتزامن Offline client وتحديث Student-visible state إلى `RELEASED` | `OWNER_DECISION` | `WORKER` | YES | YES | ODR-016 |
| **TC-AFR-013** | إفراج دفعة الممارسة التدريبية (Practice BATCH Release) | `grading manager` / `batch release engine` | نشاط تدريبي بموجه BATCH، وكافة الاستجابات معتمدة مسبقاً يدوياً (`FINALIZED`) بواسطة جهة مخولة، وحظر الإفراج قبل اكتمال manual finalization | استدعاء RPC الإفراج الجماعي بواسطة Manager الممتلك لسلطة الإفراج وتطبيق Batch completeness validation | نجاح Batch completeness validation وتدفق Batch release engine وضمان Idempotency ومعالجة الجزئية وتحديد reveal timing وتحديث Student-visible state إلى `RELEASED` وتزامن Offline client، وحظر قيام Batch worker أو Manager باعتتماد الاستجابات | `OWNER_DECISION` | `RPC` | YES | YES | ODR-016 |
