...
تنقسم مصفوفة مواصفات حالات الاختبار إلى سبع فئات هيكلية تحتوي على **94 مواصفة فريدة**:
...
7. **فئة G: بوابات التحقق وآلات الحالات (TC-GATE / TC-SM / TC-E2E-010 / TC-OBS-001)**: 11 مواصفة.

---

## 2. جدول مواصفات حالات الاختبار التفصيلي الـ 94 (Detailed 94 Test Specifications Matrix) `[REQUIRED_EXTENSION]`
...
| **TC-SCR-003** | تقديم درجة صفر مع ملاحظات توجيهية | `ordinary grader` / `reviewer` / `authorized senior grader` | إجابة غير مرتبطة بالسؤال | تقديم المصحح العادي لدرجة 0 مع الملاحظات | تحويل الحالة إلى `SUBMITTED_FOR_REVIEW` وحظر الاعتماد النهائي للمصحح العادي (DENY مع استثناء `FORBIDDEN_FINALIZATION_AUTHORITY` عند محاولة Finalize)، ثم تنفيذ الاعتماد النهائي اليدوي لاحقاً في خطوة منفصلة بواسطة Reviewer أو Authorized Senior Grader عبر `finalize_manual_grade` | `EXISTING_QB01` | `RPC` | YES | YES | ODR-007 |
...
| **TC-DMA-003** | قرار التحكيم المستقل من Senior Grader | `independent senior grader` | إجابة في طابور التحكيم (تباين > 15%)، المحكّم ليس PRIMARY_GRADER ولا COUNTERPART_GRADER ولا تضارب مصالح COI وله valid scoped assignment وexplicit arbitration capability صريحة | تقديم الدرجة المحكّمة المقترحة عبر `arbitrate_double_mark` | إنتاج proposed arbitrated score وتحديث الحالة إلى `READY_FOR_FINALIZATION` (أو `ARBITRATED_SCORE_READY_FOR_FINALIZATION`)، وتوليد سجل التدقيق (Audit event)، وحظر الاعتماد التلقائي (NO FINALIZED)، وعدم الإفراج (NO release) وعدم الإظهار (NO reveal) | `REQUIRED_EXTENSION` | `RPC` | YES | YES | ODR-008 |
...
| **TC-DMA-005** | معالجة قرار التظلم بنقله إلى الاعتماد النهائي اليدوي | `Authorized Appeal Reviewer` / `Reviewer` / `Authorized Senior Grader` | إجابة بحالة `APPEALED` وتكليف تظلم صالح لمراجع التظلم دون COI | 1. Appeal Reviewer ينفذ `grading.appeal.process` لبت التظلم. 2. فحص حفظ proposed adjusted score وprovenance وreason وaudit event وCAS validation. 3. فحص تحول حالة Response من `APPEALED` إلى `READY_FOR_FINALIZATION`. 4. التحقق من عدم الوصول لـ `FINALIZED`، وعدم الإفراج (NO release)، وعدم الكشف (NO reveal). 5. استدعاء `finalize_manual_grade` بواسطة Reviewer أو Authorized Senior Grader في خطوة منفصلة. 6. فحص تحول الحالة بعدها فقط إلى `FINALIZED`. | تطبيق قرار التظلم، حفظ proposed adjusted score، تحويل Response إلى `READY_FOR_FINALIZATION` دون اعتماد تلقائي (NO FINALIZED) ودون إفراج (NO release) ودون كشف (NO reveal)، ثم تنفيذ الاعتماد النهائي اليدوي في خطوة منفصلة بواسطة Reviewer / Authorized Senior Grader عبر `finalize_manual_grade` لتصبح `FINALIZED`. | `REQUIRED_EXTENSION` | `RPC` | YES | YES | ODR-006 |
...
| **TC-DMA-008** | التقييم المزدوج المتوافق (تسلسل احتساب المتوسط والتقريب دون اعتماد تلقائي) | `system` / `reviewer` / `authorized senior grader` | درجتان داخل حد 15% (الأول 8/10 والثاني 8.5/10) | 1. TASK-MG-046 يتحقق من variance (5%). 2. TASK-MG-046 يحسب arithmetic mean (8.25). 3. TASK-MG-034 يطبق institutional rounding عند الحاجة. 4. حفظ proposed final score. 5. تحويل الحالة إلى `READY_FOR_FINALIZATION`. 6. حظر Automatic Finalization. 7. استدعاء `finalize_manual_grade` بواسطة Reviewer / Authorized Senior Grader في خطوة منفصلة. | تنفيذ التسلسل الكامل: تطبيق TASK-MG-046 وTASK-MG-034، حفظ proposed score، تحديث الحالة إلى `READY_FOR_FINALIZATION` دون اعتماد تلقائي، ثم الاعتماد النهائي اليدوي في خطوة منفصلة لتصبح `FINALIZED` | `REQUIRED_EXTENSION` | `RPC` | YES | YES | ODR-013 |
...
| **TC-AFR-007** | الإفراج الفوري المباشر لنتائج الممارسة التدريبية (Practice IMMEDIATE Release) | `reviewer` / `authorized senior grader` / `student` | نشاط ممارسة بموجه IMMEDIATE، وإجابة معتمدة نهائياً مسبقاً (`FINALIZED`) بواسطة جهة مخولة، وعدم وجود Batch dependency | تنفيذ الإفراج الفوري عبر `TASK-MG-069` وتوليد الإشعار في Outbox | إتاحة النتيجة فوراً للطالب (Student-visible state: `RELEASED`) وتوليد Outbox بصورة حتمية ومستقلة دون انتظار الدفعة (`no batch dependency`)، مع ضمان Idempotent release وإعادة المحاولة ومزامنة Offline client | `OWNER_DECISION` | `RPC` | YES | YES | ODR-016 |
...
| **TC-AFR-012** | الإفراج المؤجل لنتائج الممارسة التدريبية (Practice DELAYED Release) | `system` / `scheduler` / `worker` | نشاط بموجه DELAYED، وإجابة معتمدة مسبقاً (`FINALIZED`)، وزمن `release_at` / `reveal_at` محفوظ بـ UTC، وعدم وجود حاجة لاكتمال الدفعة | تشغيل Worker/Scheduler عند انقضاء الوقت بـ UTC وتطبيق not-before-time guard وconfig validation | الإفراج بـ UTC وتحويل Timezone عند العرض فقط، وضمان Idempotency وRetry وFailure recovery وStale job handling وتزامن Offline client وتحديث Student-visible state إلى `RELEASED` دون أي اعتماد على الدفعة (`no batch dependency`) | `OWNER_DECISION` | `WORKER` | YES | YES | ODR-016 |
| **TC-AFR-013** | إفراج دفعة الممارسة التدريبية (Practice BATCH Release) | `grading manager` / `batch release engine` | نشاط تدريبي بموجه BATCH، وكافة الاستجابات معتمدة مسبقاً يدوياً (`FINALIZED`) بواسطة جهة مخولة، وحظر الإفراج قبل اكتمال manual finalization | استدعاء RPC الإفراج الجماعي بواسطة Manager الممتلك لسلطة الإفراج وتطبيق Batch completeness validation | نجاح Batch completeness validation وتدفق Batch release engine وضمان Idempotency ومعالجة الجزئية الفنية (partial failure handling) وتحديد reveal timing وتحديث Student-visible state إلى `RELEASED` وتزامن Offline client، وحظر قيام Batch worker أو Manager باعتتماد الاستجابات (`Worker/Manager cannot finalize Responses`, `no response finalization`) | `OWNER_DECISION` | `RPC` | YES | YES | ODR-016 |
| **TC-AFR-014** | حظر الاعتماد التلقائي للنظام والجهات غير المخولة | `ordinary grader` / `grading manager` / `admin emergency operator` / `system` / `worker` | إجابة بحالة `SUBMITTED_FOR_REVIEW` أو `READY_FOR_FINALIZATION` | محاولة تنفيذ الاعتماد النهائي (Finalization attempt) من قبل أي جهة من الجهات الخمس غير المخولة | رفض جميع محاولات الاعتماد غير المخولة (DENY)، واستثناء `FORBIDDEN_FINALIZATION_AUTHORITY`، وبقاء حالة الإجابة دون تغيير، وعدم توليد audit success event، وتسجيل security denial event، وعدم الإفراج (NO release)، وعدم الإظهار (NO reveal) | `REQUIRED_EXTENSION` | `RPC` | YES | YES | ODR-007 |
