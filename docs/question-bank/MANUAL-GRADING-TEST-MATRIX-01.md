...
> - سيتطلب كتابة حالات اختبار تنفيذية برمجية تلقائية (Automated Unit, RLS, RPC, E2E Tests).
> - سيتطلب تشغيل أدوات فحص الاختراق والتحمل التزامني (Concurrency & Security Testing).
...
5. **فئة E: الاعتماد النهائي، الإفراج الجماعي، وتوقيت Reveal (TC-AFR-001 إلى TC-AFR-011, TC-NFX-001 إلى TC-NFX-003)**: 14 مواصفة.
...
| **TC-SCR-003** | تقديم درجة صفر مع ملاحظات توجيهية | `grader` | إجابة غير مرتبطة بالسؤال | اختيار درجة 0 وتعبئة ملاحظات التقصير | قبول المراجعة بنجاح بصف `FINALIZED` | `EXISTING_QB01` | `RPC` | YES | YES | NO |
...
| **TC-QCL-004** | التحرير التلقائي عند الخمول | `system` | قفل مضى عليه 15 دقيقة دون إرسال | تشغيل job انقضاء المهل الزمني | تحويل التعيين لـ `EXPIRED` وتوفير الإجابة | `OWNER_DECISION` | `CONCURRENCY` | YES | YES | OD-MG-15 |
...
| **TC-QCL-010** | استرداد التعيين بقرار إداري (Reclaim) | `grading manager` | تعيين بطيء لم ينهِ التقييم | استدعاء RPC `reclaim_assignment` | إلغاء التعيين السابق وزيادة generation | `REQUIRED_EXTENSION` | `RPC` | YES | YES | OD-MG-15 |
...
| ID | عنوان حالة الاختبار | الدور المستهدف | الشروط المسبقة | خطوات التنفيذ | النتيجة المتوقعة | التصنيف | Future Test Layer | Requires Migration | Requires Runtime | Owner Decision Dep. |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :---: | :---: | :---: |
| **TC-DMA-001** | التصحيح المزدوج المستقل المعزول | `grader 1` / `2` | إجابة مخصصة للتصحيح المزدوج | تصحيح المصحح 1 ثم تصحيح المصحح 2 | إخفاء تقييم وملاحظات كل مصحح عن الآخر | `REQUIRED_EXTENSION` | `RLS` | YES | YES | OD-MG-14 |
...
| **TC-DMA-008** | التقييم المزدوج المتوافق (لا تباين) | `system` | درجة الأول 8/10 والثاني 8.5/10 | تسليم التقييمين بحساب تباين 5% | الاعتماد التلقائي المباشر بحالة `FINALIZED` | `REQUIRED_EXTENSION` | `RPC` | YES | YES | OD-MG-13 |
...
| **TC-AFR-007** | الاعتماد الفوري للتمارين التدريبية | `grader` | محاولة تدريب حرة طليقة | تقديم التقييم والاعتماد الفوري | تحديث درجة التمرين فوراً وإتاحة النتيجة | `OWNER_DECISION` | `RPC` | YES | YES | OD-MG-16 |
