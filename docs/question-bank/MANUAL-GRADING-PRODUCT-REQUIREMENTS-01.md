...
| `FINALIZATION_PENDING` | `FINALIZED` | `BATCH` | Batch Worker / Manager | `grading.batch.finalize` | All responses previously set to `FINALIZED` manually by authorized actors (Reviewer / Authorized Senior Grader); Batch Worker aggregates and verifies batch completeness | All responses FINALIZED check | `batch_id` | Row Lock | Batch Finalize ID | `Single Atomic RPC Transaction` | `BATCH_FINALIZED` | `FINALIZATION_FAILED, UNFINISHED_RESPONSES_IN_BATCH` | Auto Job Retry |
...
| `REOPENED` | `FINALIZATION_PENDING` | `BATCH` | Manager / System | `grading.batch.finalize` | Reopen issues resolved, all responses manually finalized by authorized actors, release blocked during reopen | CAS on `batch_version` | `expected_batch_version` | Row Lock | Batch Reopen Finalize UUID | `Single Atomic RPC Transaction` | `BATCH_REOPENED_TO_FINALIZATION_PENDING` | `BATCH_NOT_READY` | Admin Retry |
...
1. **التصحيح الأحادي (Single Grading)**: تقديم درجة واحدة من المصحح المعتمد، حفظ proposed score وحفظ score provenance وتطبيق التقريب عند الحاجة، والانتقال إلى `READY_FOR_FINALIZATION` (أو `SUBMITTED_FOR_REVIEW`)، ولا تتحول لـ `FINALIZED` آلياً، وتنتظر الاعتماد النهائي اليدوي حصرياً بواسطة Reviewer أو Authorized Senior Grader عبر `finalize_manual_grade`.
2. **التصحيح المزدوج مع التطابق التام (Exact Agreement)**: إذا كانت $\text{Score}_1 = \text{Score}_2$ تُحسب الدرجة المقترحة proposed score وتتأكد الحالة بـ `READY_FOR_FINALIZATION` دون أي اعتماد تلقائي من النظام، وتنتظر الاعتماد النهائي اليدوي بواسطة Reviewer أو Authorized Senior Grader عبر `finalize_manual_grade`.
3. **التصحيح المزدوج فوق عتبة الانحراف (Above Threshold > 15%)**: التحكيم الإجباري (Arbitration Mandatory) بواسطة `independent senior grader` عبر `arbitrate_double_mark` ينتج درجة محكّمة مقترحة بحالة `ARBITRATED_SCORE_READY_FOR_FINALIZATION` أو `READY_FOR_FINALIZATION` دون اعتماد تلقائي `FINALIZED` مباشرة، وينتظر الاعتماد النهائي اليدوي بواسطة Reviewer أو Authorized Senior Grader عبر `finalize_manual_grade`.
4. **التصحيح المزدوج ضمن عتبة الانحراف (Within Threshold $\le 15\%$):**
   - **معتمدة بقرار المالك `ODR-013` (APPROVED)**.
   - السياسة المعتمدة: التحقق من التباين عبر TASK-MG-046، احتساب المتوسط الحسابي للتقييمين (Arithmetic Mean)، وتطبيق التقريب المؤسسي عبر TASK-MG-034، وحفظ proposed final score، والانتقال إلى `READY_FOR_FINALIZATION` دون اعتماد تلقائي، وتنتظر الاعتماد النهائي اليدوي بواسطة Reviewer أو Authorized Senior Grader عبر `finalize_manual_grade`.
