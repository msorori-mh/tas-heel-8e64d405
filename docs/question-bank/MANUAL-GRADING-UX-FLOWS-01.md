...
- **إرجاع التقييم للمراجعة**: في حال وجود ملاحظات توجيهية من المصحح الأول، يتم فتح شاشة التقييم بحالة `RETURNED_FOR_SECOND_REVIEW` وتنتقل عند التعديل إلى `IN_GRADING` لعرض الملاحظات والتصحيح قبل إعادة الإرسال (`RETURNED_FOR_SECOND_REVIEW` $\to$ `IN_GRADING` $\to$ `SUBMITTED_FOR_REVIEW`).
