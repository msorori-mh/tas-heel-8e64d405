...
- Review retains `app.studentamkeen.tamkeen.review.direct` and the cached review signing key, with visible label `تمكين — أوفلاين`, versionCode 26092001. Install as an update; do not uninstall the earlier matching review app.
...
Release gate: HOLD until that exact APK passes instrumentation. No Google Play publication is performed by the review workflow.
