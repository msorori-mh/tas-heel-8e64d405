...
const migPath = join(root, "supabase", "migrations-pending", "20260820020000_quran_unit_model_correction_18e1.sql");

const dataDir = mkdtempSync(join(tmpdir(), "pg17-18e1u-"));
const sock = mkdtempSync(join(tmpdir(), "pg17-socku-"));
const env = { ...process.env, PGDATA: dataDir, PGHOST: sock, PGPORT: "55433", PGUSER: "postgres", PGDATABASE: "postgres", PGPASSWORD: "" };
const run = (c, a) => {
  const r = spawnSync(c, a, { encoding: "utf8", env });
  if (r.status !== 0) throw new Error(`${c}: ${r.stderr || r.stdout}`);
  return r.stdout;
};
const q = (sql) => spawnSync("psql", ["-v", "ON_ERROR_STOP=1", "-Atc", sql], { encoding: "utf8", env });
const one = (sql) => q(sql).stdout.trim().split("\n")[0].trim();

let started = false;
try {
  run("initdb", ["-U", "postgres", "--auth=trust"]);
  run("pg_ctl", ["-w", "-o", `-k ${sock} -p 55433 -c listen_addresses=`, "-l", join(dataDir, "log"), "start"]);
  started = true;
  ...
  const slugsBefore = one(`select md5(string_agg(slug||':'||sort_order, ',' order by slug)) from lessons`);

  r = spawnSync("psql", ["-v", "ON_ERROR_STOP=1", "-f", migPath], { encoding: "utf8", env });
  const out = [];
  const check = (name, got, want) => out.push(`${String(got) === String(want) ? "OK  " : "FAIL"}  ${name.padEnd(34)} got=${got} want=${want}`);
  ...
  check("LESSON_CODES_CHANGED", one(`select case when md5(string_agg(slug||':'||sort_order, ',' order by slug)) = '${slugsBefore}' then 0 else 1 end from lessons`), 0);

  // Guard rehearsal: a dependent row must abort the correction.
  q("insert into units (id, subject_id, title, sort_order) select gen_random_uuid(), '" + SUBJ + "', 'u'||g, g from generate_series(1,6) g");
  ...
  const guarded = spawnSync("psql", ["-v", "ON_ERROR_STOP=1", "-f", migPath], { encoding: "utf8", env });
