...
async function main() {
  ...
  // 2. Setup Baseline Prerequisite Schema & Functions
  const baselineSql = `
  ...
  console.log("[PASS] Baseline schema set up.");
  ...
  // 4. Test RLS Negative & Positive Suits
  const testSuiteSql = `
    DO $$
    DECLARE
    ...
    BEGIN
      ...
      INSERT INTO public.lessons (id, subject_id, title) VALUES (v_less, v_subj, 'Lesson 1');

      -- 4.1 TEST RLS NEGATIVE: Direct insert on operational tables denied
      PERFORM set_config('request.jwt.claim.sub', v_staff::text, true);

      -- 4.2 TEST RPC FLOW: Content staff create batch, issue upload, finalize upload
      ...
      -- Add server validation run
      INSERT INTO public.content_package_validations (version_id, batch_id, package_hash, is_valid)
      VALUES (v_version_id, v_batch_id, 'sha-hash-123', true);

      -- Submit for review
      PERFORM public.submit_resource_for_review(v_resource_id, 2, 'k-4');

      -- 4.3 TEST AUTHORIZATION: staff cannot approve or publish
      BEGIN
      ...
      -- 4.4 TEST STUDENT READ: Published resource readable via RPC
      ...
      IF jsonb_array_length(v_stu_res) <> 1 THEN
        RAISE EXCEPTION 'Student read failed to fetch published resource';
      END IF;

      -- 4.5 TEST IMMUTABILITY: Updating approved version bytes fails
      BEGIN
      ...
      -- 4.6 TEST AUDIT IMMUTABILITY: Updating review audit record fails
      BEGIN
      ...
      EXCEPTION WHEN OTHERS THEN
        -- Expected audit immutability error
      END;
  ...
  console.log("[PASS] All PG17 runtime SQL, RLS, Immutability, Storage state machine, and Idempotency tests passed successfully!");
}
