...
test("official questions require original text and a model answer", () => {
  ...
  assert.ok(missingResult.findings.some((finding) => finding.code === "MODEL_ANSWER_MISSING"));

  const valid = bytes(JSON.stringify({
    capability: "officialBookQuestions",
    questions: [{ official_text: "علل استخدام الحجر الجيري.", model_answer: "لإزالة الشوائب." }],
  }));
  assert.equal(
...
test("self-test requires options, correct answer and explanation", () => {
  const missing = bytes(JSON.stringify({
    capability: "selfTest",
    questions: [{ question: "ما رمز الحديد؟", options: ["Fe", "Cu"] }],
  }));
  const missingResult = validateGoldenLessonArtifactBytes("selfTest", "self-test.json", missing);
  assert.equal(missingResult.valid, false);
  assert.ok(missingResult.findings.some((finding) => finding.code === "SELF_TEST_CORRECT_ANSWER_MISSING"));
  assert.ok(missingResult.findings.some((finding) => finding.code === "SELF_TEST_EXPLANATION_MISSING"));
