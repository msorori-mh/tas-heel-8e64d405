...
const migration = readFileSync(
  new URL("../../supabase/migrations/20260821010000_storage_lesson_media_policy_security_definer_fix.sql", import.meta.url),
  "utf8",
);

test("lesson media lookup is isolated behind a pinned SECURITY DEFINER helper", () => {
  assert.match(migration, /CREATE OR REPLACE FUNCTION public\.can_read_lesson_media_storage_object\(_object_name text\)/);
  assert.match(migration, /SECURITY DEFINER\s+SET search_path = public, pg_temp/);
  assert.match(migration, /REVOKE ALL ON FUNCTION public\.can_read_lesson_media_storage_object\(text\) FROM PUBLIC/);
  assert.match(migration, /GRANT EXECUTE ON FUNCTION public\.can_read_lesson_media_storage_object\(text\) TO authenticated/);
...
test("storage policy no longer references lessons under the authenticated invoker", () => {
  const policyStart = migration.indexOf('CREATE POLICY "Students can read lesson media with lesson access"');
