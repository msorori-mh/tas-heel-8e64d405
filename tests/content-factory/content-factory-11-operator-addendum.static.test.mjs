...
test("ADDENDUM/2 — server inputs accept the SHA and derive the idempotency key from it", () => {
  ...
  assert.match(
    FUNCTIONS,
    /requirePlan\(data\.mode, data\.expectedPlanSha256, "CF10_WRITE_PLAN_HASH_REQUIRED"\)/,
  );
  assert.ok((FUNCTIONS.match(/requirePlan\(/g) ?? []).length >= 2);
  assert.ok(
    (FUNCTIONS.match(/idempotencyKey\("cf1[01]", data\.batchId, expected\)/g) ?? []).length >= 2,
  );
  ...
  assert.match(
    SQL,
    /_mode = 'EXECUTE' AND coalesce\(_expected_plan_sha256,''\) !~ '\^\[0-9a-f\]\{64\}\$'/,
  );
...
test("ADDENDUM/3 — batch status reads lesson_capability_lifecycle and fails closed", () => {
  ...
  assert.match(
    SERVER,
    /ok\(\s*await admin\.from\(CF11_LIFECYCLE_TABLE\)[\s\S]*?"CF11_LIFECYCLE_READ_FAILED",\s*\)/,
  );
  assert.match(
    SERVER,
...
test("ADDENDUM/4 — human stages use the operator's token, not a service-role editorial write", () => {
  ...
  assert.doesNotMatch(
    FUNCTIONS,
    /serviceClient\(\)[\s\S]{0,200}?golden_lesson_(publish|attest_cf11_ready|revoke)/,
  );
