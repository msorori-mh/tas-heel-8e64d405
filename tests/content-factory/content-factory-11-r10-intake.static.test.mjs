...
test("R10/1 — the factory UI reaches the server-side direct-file intake path", () => {
  assert.match(builder, /createGoldenLessonDirectUpload\(\{ data: \{ manifest: packageDraft \} \}\)/);
...
test("R10/2 — the client never supplies authoritative intake totals to the server", () => {
  const call = builder.slice(builder.indexOf("uploadAndVerifyDirectIntake"), builder.indexOf("return ("));
