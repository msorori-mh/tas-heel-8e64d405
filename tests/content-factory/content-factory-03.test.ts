...
} from "../../src/lib/content-factory/golden-lesson-review.ts";
import {
  parseGoldenLessonManifest,
  previewGoldenLessonStaging,
} from "../../src/lib/content-factory/golden-lesson-staging.ts";
  ...
  return {
    ...
    artifacts: GOLDEN_CAPABILITIES.map((capability) => {
      ...
      return {
        ...
        provenancePath:
          authority === "OFFICIAL" && sourcePath ? `${capability}.provenance.json` : null,
        provenanceSha256: authority === "OFFICIAL" && sourcePath ? "c".repeat(64) : null,
      };
    }),
    ...
    security: {
      ...
      answersCompanionSha256: "b".repeat(64),
...
test("review workflow enforces role and evidence without persistence", () => {
  const evidence: GoldenReviewEvidence = {
  ...
  assert.equal(
    evaluateGoldenReviewTransition("DRAFT", "SUBMITTED", "CONTENT_REVIEWER", evidence).code,
    "ROLE_FORBIDDEN",
  );
  const submitted = evaluateGoldenReviewTransition(
    "DRAFT",
    "SUBMITTED",
    "CONTENT_EDITOR",
    evidence,
  ...
  const blocked = evaluateGoldenReviewTransition(
    "SUBMITTED",
    "CONTENT_APPROVED",
    "CONTENT_REVIEWER",
    evidence,
...
test("review workflow rejects skips and keeps final approval non-executable", () => {
  ...
  assert.equal(
    evaluateGoldenReviewTransition("DRAFT", "APPROVED_FOR_STAGING", "TECHNICAL_REVIEWER", evidence)
      .code,
    "TRANSITION_INVALID",
  );
  const final = evaluateGoldenReviewTransition(
    "CONTENT_APPROVED",
