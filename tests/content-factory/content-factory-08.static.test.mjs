...
const api = readFileSync("src/lib/content-factory/golden-lesson-domain-staging.functions.ts", "utf8");
const migration = readFileSync("supabase/migrations-pending/20260819210000_content_factory_08_atomic_domain_staging.sql", "utf8");

test("CF08 maps exactly seven capabilities without live-domain mutation", () => {
  for (const target of ["lesson_book_contents","lesson_explanations","lesson_summaries","lesson_resources:mindmap","lesson_resources:experiment","questions:official","lesson_assessments:self_test"]) {
    assert.match(adapter, new RegExp(target.replace(/:/g,"\\:")));
  }
  assert.match(migration, /domain_writes_performed',0/);
  assert.doesNotMatch(migration, /INSERT INTO public\.(lesson_book_contents|lesson_explanations|lesson_summaries|lesson_resources|questions|lesson_assessments)\b/);
});
