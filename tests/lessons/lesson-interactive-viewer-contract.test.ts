...
test("student route exposes the seven-step rendering contracts", () => {
  ...
  const tabs = readFileSync("src/components/lessons/LessonCapabilityTabs.tsx", "utf8");
  assert.match(route, /<LessonCapabilityTabs/);
  ...
  assert.match(route, /const experiments = effectiveResources\.filter/);
