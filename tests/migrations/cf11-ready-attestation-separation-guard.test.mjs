...
test('non-admin self-attestation is rejected', () => {
  assert.match(sql, /IF is_admin IS NOT TRUE THEN\s+RAISE EXCEPTION 'CF11_ATTESTATION_SEPARATION_REQUIRED'/);
});
...
test('guard is fail-closed on invalid identity or failed role lookup', () => {
  ...
  assert.match(sql, /EXCEPTION WHEN OTHERS THEN\s+RAISE EXCEPTION 'CF11_ATTESTATION_ROLE_CHECK_FAILED'/);
});
...
test('guard runs on both insert and update, security definer, pinned search_path', () => {
  ...
  assert.match(sql, /FOR EACH ROW EXECUTE FUNCTION public\.cf11_guard_ready_attestation_separation\(\)/);
