-- 21B-A3 — local PG17 baseline reproducing the CURRENT production schema of
-- public.subject_textbooks (post 21B-A2 apply). No production data involved.
...
CREATE TABLE public.subject_textbooks (
  ...
  coverage_type text NOT NULL DEFAULT 'FULL_ACADEMIC_YEAR',
  CONSTRAINT subject_textbooks_bucket_allowed CHECK (storage_bucket = 'lesson-pdfs'),
  CONSTRAINT subject_textbooks_coverage_type_valid
  ...
  CONSTRAINT subject_textbooks_semester_valid CHECK (
    (coverage_type = 'FULL_ACADEMIC_YEAR' AND semester IS NULL)
    OR (coverage_type = 'SEMESTER_SPECIFIC' AND semester IS NOT NULL AND semester IN (1, 2))
  ),
