...
test("R2 visibility output contract is PG17 type-safe", () => {
  const output = diff.slice(diff.indexOf("SELECT 'EXPECTED_GAIN_COUNT'"), diff.indexOf("-- A zero result"));
  for (const counter of ["expected_gain_count", "security_fix_count", "unexpected_gain_count", "unexpected_loss_count"]) {
...
test("R2 reveal preserves revision pinning and prevents answer/rationale leakage", () => {
  ...
  for (const answerKey of ["correct_index", "is_correct", "model_answer", "explanation", "why_correct", "why_wrong"]) {
    assert.doesNotMatch(initial, new RegExp(`\\b${answerKey}\\b`, "i"), answerKey);
  }
  assert.match(reveal, /ANSWER_NOT_AVAILABLE/i);
});

test("visibility diff runs against an available local PostgreSQL 17 target", { skip: !localPg17Url(process.env.TAMKEEN_PG17_LOCAL_URL ?? "") }, () => {
  ...
  assert.doesNotMatch(`${result.stdout}\n${result.stderr}`, /UNION types bigint and text cannot be matched|SQLSTATE=42804/i);
});
