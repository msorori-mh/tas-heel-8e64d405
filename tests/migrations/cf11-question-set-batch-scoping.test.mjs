...
function findMigration() {
  ...
  const hits = files.filter((f) =>
    readFileSync(join(MIG_DIR, f), "utf8").includes(
      "CF11_SCOPING_MIGRATION_UNEXPECTED_OCCURRENCES",
    ),
  );
...
test("official and selfTest use the same batch-scoping rule", () => {
  const off =
    sql.match(/lesson_id = lesson_row\.id AND code = ANY \(expected_official_codes\)/g) ?? [];
  const self =
    sql.match(/lesson_id = lesson_row\.id AND code = ANY \(expected_self_codes\)/g) ?? [];
...
test("historical extra DRAFT codes cannot pollute the live set (semantic model)", () => {
  ...
  const expectedSelf = Array.from(
    { length: 20 },
    (_, i) => `SELF st-chem-g12-iron-v2-${String(i + 1).padStart(5, "0")}`,
  ).sort();
  ...
  const missing = lessonRows
    .filter((c) => c !== expectedSelf[0] && expectedSelf.includes(c))
    .sort();
