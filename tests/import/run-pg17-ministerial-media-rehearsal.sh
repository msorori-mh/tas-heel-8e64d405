#!/usr/bin/env bash
# MINISTERIAL_QUESTION_MEDIA_V1 — disposable PostgreSQL 17 rehearsal.
#
# Replays the ministerial chain, a local stand-in for Supabase Storage, the
# 20260913 admin baseline, and then the PENDING media migration, before running
# the media runtime contract. The caller must provide a clean local database;
# remote hosts are rejected so this can never touch production.
set -euo pipefail
...
MEDIA_MIGRATION="supabase/migrations-pending/20260914010000_ministerial_question_media.sql"
if [ ! -f "$ROOT/$MEDIA_MIGRATION" ]; then
  # Once promoted, the same file lives under supabase/migrations.
  MEDIA_MIGRATION="supabase/migrations/20260914010000_ministerial_question_media.sql"
fi
...
CHAIN=(
  ...
  tests/import/fixtures/pg17-prereq-ministerial-media.sql
...
output="$(psql "$MINISTERIAL_PG17_URL" --set=ON_ERROR_STOP=1 --file \
  tests/import/fixtures/pg17-ministerial-media-smoke.sql 2>&1)"
psql_rc=$?
set -e
echo "$output"
...
if [ "$pass_count" -lt 80 ]; then
  echo "Ministerial media rehearsal produced $pass_count PASS assertions; expected at least 80." >&2
  exit 1
fi
echo "RESULT: ministerial question media PostgreSQL 17 rehearsal = PASS ($pass_count assertions)"
