...
# REPEATED_MINISTERIAL_QUESTIONS_14G — local PostgreSQL 17 rehearsal
#
...
createdb -h "$SOCK" -U postgres t14fg
for f in "${CHAIN[@]}"; do
...
if [ "$PASS_COUNT" -lt 18 ]; then
  echo "FAIL  smoke produced only $PASS_COUNT PASS lines"
  FAILURES=$((FAILURES + 1))
fi

if [ "$FAILURES" -eq 0 ]; then
  echo; echo "RESULT: 14F/14G REHEARSAL = PASS"; exit 0
else
  echo; echo "RESULT: 14F/14G REHEARSAL = FAIL ($FAILURES failures)"; exit 1
fi
