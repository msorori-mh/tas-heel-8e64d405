...
function fixture(cap = 14, failAt = -1, emptyAt = -1) {
  const lessons = Array.from({ length: 20 }, (_, i) => ["fiqh", "quran"].map(subject => ({
    id: `${String(i).padStart(3, "0")}-${subject}`, slug: `${subject}-${i + 1}`,
    title: subject === "fiqh" && i === 19 ? "التعزير" : `${subject} ${i + 1}`,
    subject_id: subject, unit_id: null, semester: 1, sort_order: i + 1,
  }))).flat();
  ...
  const client = {
    from(table: string) {
      ...
      const query = {
        ...
        range(start: number, end: number) { from = start; to = end; offsets.push(start); return query; },
        then(resolve: (value: unknown) => unknown) {
          const data = table === "lessons"
            ? (from >= emptyAt && emptyAt >= 0 ? [] : lessons.slice(from, Math.min(to + 1, from + cap)))
            : table === "subjects" ? ["fiqh", "quran"].map(id => ({ id, code: id, name: id })) : [];
          return Promise.resolve({ data, count: lessons.length,
            error: table === "lessons" && failAt >= 0 && from >= failAt ? { message: "page failed" } : null,
          }).then(resolve);
        },
      };
      return query;
  ...
  return { client: client as unknown as Parameters<typeof loadContentCodeRegistry>[0], offsets, orders };
}
...
describe("complete import lesson registry", () => {
  it("retains all 20 lessons per subject even when the server caps each response at 14", async () => {
    ...
    for (const subject of ["fiqh", "quran"]) {
      const visible = registry.lessons.filter(l => l.subjectCode === subject && l.unitCode === null);
