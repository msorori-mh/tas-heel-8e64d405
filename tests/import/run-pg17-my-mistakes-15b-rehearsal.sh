#!/usr/bin/env bash
# =============================================================================
# TAMKEEN_MY_MISTAKES_DERIVED_MODEL_15B — local PostgreSQL 17 rehearsal
#
# Disposable cluster only; never touches the shared datastore.
#   bash tests/import/run-pg17-my-mistakes-15b-rehearsal.sh
# =============================================================================
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
RUN="${TMPDIR:-/tmp}/pg17-15b"
...
createdb -h "$SOCK" -U postgres t15b
for f in "${CHAIN[@]}"; do
  "${PSQL[@]}" -d t15b -f "$f" >/dev/null
done

"${PSQL[@]}" -d t15b -f "$PREREQ" >/dev/null
"${PSQL[@]}" -d t15b -f "$MIG_14FG" >/dev/null

echo "== applying 15B"
"${PSQL[@]}" -d t15b -f "$MIG_15B" >/dev/null
echo "== applying 15B again (idempotency)"
"${PSQL[@]}" -d t15b -f "$MIG_15B" >/dev/null

echo "== runtime smoke"
OUT="$(psql -h "$SOCK" -U postgres -d t15b -qtA -f tests/import/fixtures/pg17-my-mistakes-15b-smoke.sql 2>&1 || true)"
...
if [ "$FAILURES" -eq 0 ]; then
  echo; echo "RESULT: 15B REHEARSAL = PASS"; exit 0
else
  echo; echo "RESULT: 15B REHEARSAL = FAIL ($FAILURES failures)"; exit 1
fi
