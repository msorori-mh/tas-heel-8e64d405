...
const hash = (file) =>
  crypto
    .createHash("sha256")
    .update(fs.readFileSync(intakePath(file)))
    .digest("hex");
...
test("official content has exact 20A structured blocks, ordered pages, figures, equations and source hash", (t) => {
  ...
  assert.deepEqual(
    x.blocks.filter((b) => b.type === "table").map((b) => b.id),
    ["iron-family"],
  );
  ...
  assert.match(
    read("official-content.html"),
    new RegExp('data-source-file-hash="sha256:' + h + '"'),
  );
...
test("official equations retain balanced coefficients, arrows, conditions and ΔH", () => {
  const equations = new Map(
    json("official-content.json")
      .blocks.filter((b) => b.type === "equation")
      .map((b) => [b.id, b.text]),
  );
  ...
  assert.equal(
    equations.get("carbon-dioxide-formation"),
    "C(s) + O₂(g) → CO₂(g)  ΔH= −394 Kj/mole",
  );
  assert.equal(equations.get("carbon-monoxide-formation"), "CO₂(g) + C(s) → 2CO(g)  ΔH= +173 Kj");
  assert.match(
    read("official-content.html"),
    /3Fe<sub>2<\/sub>O<sub>3<\/sub> \+ CO → 2Fe<sub>3<\/sub>O<sub>4<\/sub>/,
  );
...
for (const file of ["explanation.html", "summary.html"])
  test(file + " is static RTL HTML", () => {
    const s = read(file);
...
test("lab uses interactive sandbox contract with zero external network dependency", () => {
  ...
  assert.doesNotMatch(
    s,
    /onclick\s*=|fetch\s*\(|XMLHttpRequest|WebSocket|EventSource|window\.parent|https?:\/\//i,
  );
...
test("official question filter excludes unrelated unit questions", () => {
  ...
  assert.deepEqual(
    x.questions.map((q) => q.question_number),
    ["7", "8", "9", "10", "11a-d"],
  );
...
test("readiness and previews expose the real review gaps", () => {
  ...
  for (const c of caps) {
    assert.match(student, new RegExp('id="' + c + '"'));
    assert.match(admin, new RegExp(c));
  }
...
test("provenance covers every capability", () => {
  ...
  for (const c of caps) {
    assert.ok(p[c].source_file);
    assert.ok(p[c].source_sha256.startsWith("sha256:"));
    assert.ok(p[c].review_status);
  }
  ...
  for (const c of [
    ...
    assert.equal(p[c].content_owner, "TAMKEEN");
