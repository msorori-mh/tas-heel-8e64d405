...
try {
  for (const viewport of viewports) {
    ...
    check(
      (await page.locator("[data-capability]").count()) === 7,
      viewport.name + ": seven capabilities",
    );
    ...
    check(
      ...
      viewport.name + ": no outer horizontal overflow",
    );
    await page.getByRole("button", { name: "أسئلة الكتاب" }).click();
    await page.locator("#official-list .card").first().waitFor();
    check(
      (await page.locator("#official-list .card").count()) === 5,
      viewport.name + ": five official question groups",
    );
    await page.getByRole("button", { name: "الاختبار الذاتي" }).click();
    await page.locator("#self-list .card").first().waitFor();
    check(
      (await page.locator("#self-list .card").count()) === 20,
      viewport.name + ": 20 self-test questions",
    );
    check(
      (await page
        .locator("#selfTest")
        .getByText(/correct_option|rationale|answer_key/i)
        .count()) === 0,
      viewport.name + ": no answer leak",
    );
  ...
  check(
    (await official.locator("table thead tr").count()) === 2,
    "official table has two header rows",
  );
  check(
    (await official.locator("table tbody tr").count()) === 3,
    "official table has three element rows",
  );
  check((await official.locator(".equation").count()) >= 15, "official equations complete");
  check(
    (await official.locator('.equation:has-text("394")').count()) === 1 &&
      (await official.locator('.equation:has-text("173")').count()) === 1,
    "official thermochemistry equations visible",
  );
  const image = official.locator('[data-block-id="furnace-figure"] img');
  await image.waitFor();
  check(
    await image.evaluate((img) => img.complete && img.naturalWidth > 0),
    "actual furnace figure loads",
  );
  ...
  const targets = await mind
    .locator("summary")
    .evaluateAll((nodes) => nodes.map((n) => n.getBoundingClientRect().height));
  check(
    targets.every((height) => height >= 44),
    "mindmap touch targets are at least 44px",
  );
  const mindSize = await mind.evaluate(() => ({
  ...
  const cspErrors = [],
    external = [];
  lab.on("console", (msg) => {
    if (/content security policy|refused to/i.test(msg.text())) cspErrors.push(msg.text());
  });
  lab.on("request", (request) => {
  ...
  await lab.selectOption("#ion", { label: "Fe2+" });
  await lab.click("#run");
  ...
  await lab.selectOption("#ion", { label: "Fe3+" });
  await lab.click("#run");
...
if (failures.length) {
  console.error("IRON_UI_RUNTIME_FAIL");
  for (const failure of failures) console.error("- " + failure);
  process.exitCode = 1;
} else {
  console.log("IRON_UI_RUNTIME_PASS viewports=4 capabilities=7 self_test=20 csp=PASS network=0");
}
