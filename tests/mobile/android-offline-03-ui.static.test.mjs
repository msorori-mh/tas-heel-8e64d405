...
const library = read("src/lib/offline/offline-download-library.ts");
...
describe("OFFLINE-03 truthful student UI", () => {
  it("uses the attested subject manifest instead of the legacy PDF-only pack", () => {
    expect(subjectRoute).not.toContain("OfflineSubjectPackCard");
    expect(subjectRoute).not.toContain("<OfflinePackCard");
    expect(library).toContain("prepareOfflineSubjectPack");
    expect(library).toContain("downloadOfflineSubjectPack");
    expect(library).toContain("inspectOfflineSubjectPack");
    expect(settings).toContain("استكمال التنزيل");
  ...
  it("does not claim restart-in-airplane-mode closure before the embedded-app gate", () => {
    ...
    expect(settings).not.toContain("بعد إعادة تشغيل التطبيق");
  });
});
