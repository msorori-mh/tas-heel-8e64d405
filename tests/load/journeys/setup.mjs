...
try {
  ...
  const schema = await readFile(
    "tests/offline/fixtures/pg17-offline-assessment-schema.sql",
    "utf8",
  );
  ...
  const body =
    '<article dir="rtl">TEST_ONLY محتوى الدرس<img alt="TEST_ONLY" src="data:image/png;base64,' +
    image +
    '"></article><!--' +
    randomBytes(786432).toString("base64") +
    "-->";
  ...
  await db.query(
    "UPDATE lesson_capability_lifecycle c SET ready_hash=b.offline_metadata_v1->>'sha256' FROM lesson_book_contents b WHERE b.lesson_id=c.lesson_id AND c.capability='officialBookContent'",
  );
  await db.query("ANALYZE");
  const counts = (
    await db.query(
      "SELECT (SELECT count(*) FROM auth.users)::int users,(SELECT count(*) FROM lessons)::int lessons,(SELECT count(*) FROM questions)::int questions,(SELECT count(*) FROM lesson_book_contents)::int books,pg_database_size(current_database())::bigint database_bytes",
    )
  ).rows[0];
  await mkdir("artifacts/journeys", { recursive: true });
  await writeFile(
    "artifacts/journeys/fixture.json",
    JSON.stringify(
      {
        scope: "TEST_ONLY generated users/content; not a production copy",
        ...counts,
        bookBodyBytes: Buffer.byteLength(body),
      },
      null,
      2,
    ),
