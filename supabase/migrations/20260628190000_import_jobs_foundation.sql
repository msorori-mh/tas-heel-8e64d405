...
-- Tables record dry-run/validation/execute lifecycle, row counts, and per-row errors/warnings.
...
CREATE TABLE public.import_jobs (
  ...
  CONSTRAINT import_jobs_mode_check CHECK (
  ...
  CONSTRAINT import_jobs_file_size_nonneg CHECK (
    file_size_bytes IS NULL OR file_size_bytes >= 0
  )
);

COMMENT ON TABLE public.import_jobs IS
  'Tracks import operations (preview, validation, execute). Does not perform imports itself.';
COMMENT ON COLUMN public.import_jobs.created_by IS
  'Admin user who initiated the import job.';
COMMENT ON COLUMN public.import_jobs.summary IS
  'Aggregated validation/execute results (counts, sheet breakdown, etc.).';
COMMENT ON COLUMN public.import_jobs.metadata IS
  'Opaque job context (upload refs, parser version, options).';
...
CREATE TABLE public.import_errors (
  ...
  CONSTRAINT import_errors_severity_check CHECK (
    severity IN ('error', 'warning')
  ),
  CONSTRAINT import_errors_row_number_pos CHECK (
    row_number IS NULL OR row_number >= 1
  )
);

COMMENT ON TABLE public.import_errors IS
  'Per-row validation errors and warnings for an import job. Does not perform imports itself.';
COMMENT ON COLUMN public.import_errors.row_data IS
  'Snapshot of the source row; may contain sensitive values — admin-only via RLS.';
COMMENT ON COLUMN public.import_errors.error_code IS
  'Stable machine-readable code for grouping and UI display.';
