...
-- New student path: wallet_topup_request -> admin approve/reject -> wallet credit -> pay_subscription_from_wallet
...
CREATE INDEX IF NOT EXISTS idx_wallet_topup_requests_user_id
  ON public.wallet_topup_requests (user_id);
CREATE INDEX IF NOT EXISTS idx_wallet_topup_requests_status
  ON public.wallet_topup_requests (status);
CREATE INDEX IF NOT EXISTS idx_wallet_topup_requests_payment_method_id
  ON public.wallet_topup_requests (payment_method_id);
CREATE INDEX IF NOT EXISTS idx_wallet_topup_requests_created_at_desc
  ON public.wallet_topup_requests (created_at DESC);
CREATE INDEX IF NOT EXISTS idx_wallet_topup_requests_reviewed_by
  ON public.wallet_topup_requests (reviewed_by)
  WHERE reviewed_by IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_wallet_topup_requests_credited_transaction_id
  ON public.wallet_topup_requests (credited_transaction_id)
...
-- Creation via create_wallet_topup_request RPC only (no client INSERT/UPDATE/DELETE policies).

-- ===== RPC: create_wallet_topup_request =====
...
-- ===== RPC: approve_wallet_topup_request =====
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'wallet_topup_request_not_found' USING ERRCODE = 'P0002';
  END IF;
  ...
  IF v_req.status = 'credited' THEN
    RETURN jsonb_build_object(
      'request_id', v_req.id,
      'status', v_req.status,
      'wallet_account_id', v_req.wallet_account_id,
      'credited_transaction_id', v_req.credited_transaction_id,
      'already_processed', true
    );
  END IF;

  IF v_req.status = 'rejected' THEN
    RAISE EXCEPTION 'wallet_topup_request_already_rejected' USING ERRCODE = '22023';
  END IF;
  ...
  IF v_req.wallet_account_id IS NULL THEN
    ...
    SET wallet_account_id = v_req.wallet_account_id, updated_at = now()
    WHERE id = v_req.id;
  END IF;

  v_deposit := public.create_wallet_transaction(
    _user_id := v_req.user_id,
    _type := 'deposit',
    _direction := 'credit',
    _amount := v_req.amount,
    _currency := v_req.currency,
    _reference_type := 'wallet_topup',
    _reference_id := v_req.id,
    _description := 'Wallet top-up approved',
    _metadata := jsonb_build_object(
      'wallet_topup_request_id', v_req.id,
      'payment_method_id', v_req.payment_method_id,
      'receipt_path', v_req.receipt_path,
      'receipt_hash', v_req.receipt_hash,
  ...
  SET
    ...
    credited_transaction_id = v_existing_tx,
    updated_at = now()
  WHERE id = v_req.id;

  PERFORM public.write_audit_log(
    'wallet_topup.approved',
    ...
    jsonb_build_object(
      ...
      'deposit_transaction_id', v_existing_tx,
      'wallet_balance_after', (v_deposit->>'balance_after')::numeric,
...
-- ===== RPC: reject_wallet_topup_request =====
...
BEGIN
  ...
  IF v_req.status = 'credited' THEN
    RAISE EXCEPTION 'credited_request_cannot_be_rejected' USING ERRCODE = '22023';
  END IF;
...
-- ===== Storage: extend receipt lock policies for wallet top-ups =====
...
CREATE POLICY "Users can delete own receipts when not under review"
  ...
  USING (
    ...
    AND NOT EXISTS (
      ...
      WHERE wtr.user_id = auth.uid()
        AND wtr.receipt_path = storage.objects.name
