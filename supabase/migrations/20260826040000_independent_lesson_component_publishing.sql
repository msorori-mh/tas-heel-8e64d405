-- Independent publication guard for the seven Content V3 lesson components.
-- A content mutation must never become student-visible merely because a source
-- row exists. The mutation and lifecycle downgrade share one DB transaction:
-- if the guard fails, the content write rolls back.
BEGIN;
...
DECLARE
  ...
  _status text := CASE WHEN TG_OP = 'DELETE' THEN OLD.status ELSE NEW.status END;
  _label text := CASE
    WHEN TG_OP = 'DELETE' THEN OLD.educational_label
    ELSE NEW.educational_label
  END;
  _lesson_id uuid;
  _capability text;
BEGIN
...
-- Rollback:
-- DROP TRIGGER ... on the six source tables, then drop the three guard trigger
-- functions and mark_lesson_component_draft(uuid,text). No content rows or
-- lifecycle history need deletion.
