  ...
  SELECT jsonb_build_object(
    'summary', jsonb_build_object(
      ...
      'improvement_percentage_points',
        (SELECT CASE WHEN previous_avg IS NULL OR recent_avg IS NULL THEN NULL
    ...
    'by_mode', coalesce((
      ...
      FROM (
        ...
        FROM finals
        GROUP BY coalesce(attempt_mode, 'training')
      ) t
    ), '[]'::jsonb),
    'by_subject', coalesce((
    ...
    'by_lesson', coalesce((
      SELECT jsonb_agg(jsonb_build_object(
               ...
               'accuracy', accuracy
             ) ORDER BY asked DESC, lesson_title)
      FROM by_lesson_acc
    ), '[]'::jsonb),
    'weak_lessons', coalesce((
      SELECT jsonb_agg(jsonb_build_object(
             ...
             ) ORDER BY accuracy ASC, asked DESC)
      FROM by_lesson_acc
      WHERE accuracy IS NOT NULL AND accuracy < 60
    ), '[]'::jsonb),
    'patterns', (
      SELECT jsonb_build_object(
        ...
        'wrong_rate', CASE WHEN count(*) > 0 THEN round(
          (count(*) FILTER (WHERE is_correct IS FALSE AND answered_at IS NOT NULL)::numeric
           / count(*)) * 100, 1) ELSE NULL END,
