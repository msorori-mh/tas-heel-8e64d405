...
-- 6.3 mind map (HTML pipeline) — production lesson_resources has no
--     lifecycle_status column yet, so every mind map present today is already
--     student-visible and is grandfathered to READY (visibility preserved).
INSERT INTO public.lesson_capability_lifecycle
  (lesson_id, capability, status, ready_at)
