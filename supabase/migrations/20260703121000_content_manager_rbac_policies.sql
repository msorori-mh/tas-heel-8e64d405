-- CONTENT-MANAGER-RBAC-01A: content-staff helpers + RLS (requires content_manager enum)
-- Does NOT touch payment/wallet/subscription/profiles admin view.

-- ============================================================
-- 1) Role helpers (do not modify has_role)
-- ============================================================
...
-- ============================================================
-- 2) Content table RLS — replace admin-only manage policies
-- ============================================================

-- grades, subjects, lessons, questions (03842)
DROP POLICY IF EXISTS "Admins can manage grades" ON public.grades;
CREATE POLICY "Content staff manage grades"
...
CREATE POLICY "Content staff manage subjects"
  ON public.subjects FOR ALL TO authenticated
  USING (public.is_content_staff(auth.uid()))
  WITH CHECK (public.is_content_staff(auth.uid()));

DROP POLICY IF EXISTS "Admins can manage lessons" ON public.lessons;
CREATE POLICY "Content staff manage lessons"
  ON public.lessons FOR ALL TO authenticated
  USING (public.is_content_staff(auth.uid()))
  WITH CHECK (public.is_content_staff(auth.uid()));

DROP POLICY IF EXISTS "Admins can manage questions" ON public.questions;
CREATE POLICY "Content staff manage questions"
  ON public.questions FOR ALL TO authenticated
  USING (public.is_content_staff(auth.uid()))
  WITH CHECK (public.is_content_staff(auth.uid()));

-- curriculum structure (04917)
DROP POLICY IF EXISTS "Admins manage tracks" ON public.curriculum_tracks;
CREATE POLICY "Content staff manage tracks"
  ON public.curriculum_tracks FOR ALL TO authenticated
  USING (public.is_content_staff(auth.uid()))
  WITH CHECK (public.is_content_staff(auth.uid()));

DROP POLICY IF EXISTS "Admins manage governorates" ON public.governorates;
CREATE POLICY "Content staff manage governorates"
...
CREATE POLICY "Content staff manage map"
  ON public.governorate_curriculum_map FOR ALL TO authenticated
  USING (public.is_content_staff(auth.uid()))
  WITH CHECK (public.is_content_staff(auth.uid()));

DROP POLICY IF EXISTS "Admins manage units" ON public.units;
CREATE POLICY "Content staff manage units"
  ON public.units FOR ALL TO authenticated
  USING (public.is_content_staff(auth.uid()))
  WITH CHECK (public.is_content_staff(auth.uid()));

DROP POLICY IF EXISTS "Admins manage book contents" ON public.lesson_book_contents;
CREATE POLICY "Content staff manage book contents"
  ON public.lesson_book_contents FOR ALL TO authenticated
  USING (public.is_content_staff(auth.uid()))
  WITH CHECK (public.is_content_staff(auth.uid()));

DROP POLICY IF EXISTS "Admins manage explanations" ON public.lesson_explanations;
CREATE POLICY "Content staff manage explanations"
  ON public.lesson_explanations FOR ALL TO authenticated
  USING (public.is_content_staff(auth.uid()))
  WITH CHECK (public.is_content_staff(auth.uid()));

DROP POLICY IF EXISTS "Admins manage assessments" ON public.lesson_assessments;
CREATE POLICY "Content staff manage assessments"
  ON public.lesson_assessments FOR ALL TO authenticated
  USING (public.is_content_staff(auth.uid()))
  WITH CHECK (public.is_content_staff(auth.uid()));
...
CREATE POLICY "Content staff manage assessment questions"
...
CREATE POLICY "Content staff manage simulations"
  ON public.lesson_simulations FOR ALL TO authenticated
  USING (public.is_content_staff(auth.uid()))
  WITH CHECK (public.is_content_staff(auth.uid()));

-- lesson_summaries: keep service_role policy; add content staff manage
CREATE POLICY "Content staff manage summaries"
  ON public.lesson_summaries FOR ALL TO authenticated
  USING (public.is_content_staff(auth.uid()))
  WITH CHECK (public.is_content_staff(auth.uid()));

-- exam templates (07234143)
...
CREATE POLICY "Content staff manage templates"
...
CREATE POLICY "Content staff manage template questions"
...
CREATE POLICY "Content staff manage import jobs"
...
-- ============================================================
-- 4) Storage — lesson-pdfs & lesson-videos only
-- ============================================================
DROP POLICY IF EXISTS "Admins manage lesson files - select" ON storage.objects;
CREATE POLICY "Content staff manage lesson files - select"
  ON storage.objects FOR SELECT TO authenticated
  USING (
    bucket_id IN ('lesson-pdfs', 'lesson-videos')
    AND public.is_content_staff(auth.uid())
  );
...
CREATE POLICY "Content staff manage lesson files - update"
...
CREATE POLICY "Content staff manage lesson files - delete"
...
-- user_roles remains service_role only (no changes here)
