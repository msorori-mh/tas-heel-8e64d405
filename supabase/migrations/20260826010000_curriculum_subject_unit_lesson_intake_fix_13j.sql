-- CURRICULUM_SUBJECT_UNIT_LESSON_INTAKE_FIX_13J
-- Makes the curriculum intake contract explicit and fail-closed:
--   * every imported subject resolves one real grade;
--   * units are optional at subject level but strict when imported;
--   * a lesson accepts an empty unit_code (direct subject attachment);
--   * a non-empty unknown unit_code is rejected instead of silently dropped;
--   * updating a lesson can move it from a unit back to the subject.
...
BEGIN
  ...
  LOOP
    ...
    IF NULLIF(btrim(p->>'unit_code'), '') IS NOT NULL AND subject IS NOT NULL THEN
      SELECT u.id INTO unit FROM public.units u
      WHERE u.subject_id = subject AND u.code = btrim(p->>'unit_code');
    END IF;

    CASE _template_key
      WHEN 'subjects' THEN
        entity_type := 'subjects';
        SELECT g.id INTO grade FROM public.grades g WHERE g.slug = btrim(p->>'grade_slug');
        IF grade IS NULL THEN
          RAISE EXCEPTION 'GRADE_NOT_FOUND: %', p->>'grade_slug' USING ERRCODE = '23503';
        END IF;
        SELECT s.id INTO target FROM public.subjects s WHERE s.code = p->>'subject_code';
        IF target IS NOT NULL AND NOT EXISTS (
          SELECT 1 FROM public.subjects s WHERE s.id = target AND s.grade_id = grade
        ) THEN
          RAISE EXCEPTION 'SUBJECT_GRADE_MISMATCH: % / %', p->>'subject_code', p->>'grade_slug'
            USING ERRCODE = '23514';
        END IF;
      WHEN 'units' THEN
        entity_type := 'units';
        ...
        END IF;
        IF NULLIF(btrim(p->>'unit_code'), '') IS NULL THEN
          RAISE EXCEPTION 'UNIT_CODE_REQUIRED: row %', row_rec.row_number USING ERRCODE = '23514';
        END IF;
      ...
      WHEN 'lessons' THEN
        ...
        IF subject IS NULL THEN
          RAISE EXCEPTION 'SUBJECT_NOT_FOUND: %', p->>'subject_code' USING ERRCODE = '23503';
        END IF;
        IF NULLIF(btrim(p->>'unit_code'), '') IS NOT NULL AND unit IS NULL THEN
          RAISE EXCEPTION 'UNIT_NOT_FOUND_FOR_SUBJECT: % / %', p->>'subject_code', p->>'unit_code'
            USING ERRCODE = '23503';
        END IF;
    ...
    CASE _template_key
      WHEN 'subjects' THEN
        IF target IS NULL THEN
          INSERT INTO public.subjects (code, slug, name, group_code, group_name, grade_id, curriculum_track_id, semester, icon, color, sort_order)
          VALUES (
            p->>'subject_code',
            p->>'slug',
            p->>'name',
      ...
      WHEN 'lessons' THEN
        ...
        ELSE
          UPDATE public.lessons SET
            -- Blank unit_code intentionally moves the lesson to direct subject
            -- attachment; a non-blank unknown unit was rejected above.
            unit_id = unit,
            title = p->>'title',
...
REVOKE ALL ON FUNCTION public._qb_import_row_hash(jsonb) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public._qb_import_content_fingerprint(jsonb) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public._qb_import_row_hash(jsonb) TO service_role;
GRANT EXECUTE ON FUNCTION public._qb_import_content_fingerprint(jsonb) TO service_role;
