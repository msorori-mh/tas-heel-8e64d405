...
BEGIN
  ...
  counts := jsonb_build_object(
    ...
    'question_revisions',      coalesce(array_length(revision_ids, 1), 0),
    'question_targets',        (SELECT count(*) FROM public.question_targets x WHERE x.question_id = ANY(question_ids)),
    'question_options',        (SELECT count(*) FROM public.question_options x WHERE x.question_revision_id = ANY(revision_ids)),
