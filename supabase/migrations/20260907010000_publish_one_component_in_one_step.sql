-- ============================================================================
-- نشر مكوّن واحد في خطوة واحدة
-- ============================================================================
--
-- المطلوب: اختيار الملف ← فحصه ← نشره. ثلاث خطوات، ويظهر المكوّن للطالب فورًا.
--
-- لماذا دالة جديدة بدل تعديل CF11:
--   golden_lesson_publish_cf11 تنشر الدرس وحدةً واحدة: تطلب الخريطة الذهنية
--   والتجربة المعملية ومحتوى الكتاب في كل دفعة، وتنقل المكوّنات السبعة معًا إلى
--   REVIEW، وترفض النشر إن كان أي مكوّن قد اعتُمد من قبل. تفكيك ثلاثة وخمسين
--   حارسًا متشابكًا فيها أخطر بكثير من كتابة مسار مباشر بجانبها. تبقى CF11 كما
--   هي لمن يريد النشر الجماعي؛ وهذه الدالة هي المسار البسيط.
--
-- ما تفعله لمكوّن واحد:
--   1. تتحقّق من الصلاحية ومن أن الدفعة تحمل هذا المكوّن فعلًا.
--   2. تفحص سلامته: لا تسريب إجابات، وعقد HTML التفاعلي للخريطة والتجربة.
--   3. تُنتج ما يحتاجه الطالب لرؤيته:
--        • الكتاب والشرح والملخّص: CF10 كتبتها بالفعل، فتتأكّد من وجود الصف.
--        • الخريطة والتجربة: تُنشئ صف lesson_resources بالمحتوى المضمّن.
--        • الأسئلة: تنشر النسخة المثبّتة وتضبط مؤشّر النشر، وتملأ التقييم.
--   4. تضع صف دورة الحياة الخاص به على READY.
--
-- ما لا تفعله عمدًا — وهذا ما طُلب إلغاؤه:
--   لا مرحلة REVIEW وسيطة، ولا فصل مسؤوليات، ولا انتظار لبقية المكوّنات، ولا
--   تعطيل بسبب مكوّن آخر منشور أو مفقود.
--
-- ما أبقيتُه عمدًا رغم طلب التبسيط — لأنه ليس تعقيدًا إجرائيًا بل صحّة:
--   • منع تسريب الإجابات: بدونه تظهر الإجابات للطالب داخل المحتوى.
--   • عقد HTML: بدونه يصير ملف مرفوع بابًا لتنفيذ سكربت في متصفّح الطالب.
--   • ربط الهوية CF09: بدونه قد يُنشر محتوى في درس آخر.
--   • مطابقة البصمة: المنشور هو نفسه المفحوص بايت ببايت.
--
-- الأثر: مكوّن واحد يظهر للطالب وحده. سياسات الصفوف تسأل عن كل مكوّن على حدة
-- أصلًا (lesson_capability_ready)، فلا تحتاج طبقة الطالب أي تعديل.
-- ============================================================================
...
BEGIN
  ...
  -- الدفعة تحمل هذا المكوّن فعلًا: بلا ملف لا يوجد ما يُنشر.
  SELECT * INTO entry
  ...
  -- ربط الهوية CF09: درس واحد بعينه، لا اجتهاد.
  ...
  -- CF10 لا بد أن تكون وطّنت هذه الدفعة، وإلا فلا محتوى في جداول الدرس.
  ...
  -- 2) السلامة ------------------------------------------------------------------
  -- تسريب الإجابات يُفحص لكل مكوّن نصّي بلا استثناء.
  ...
  ELSIF _capability IN ('mindMapHtml', 'labExperimentHtml') THEN
    -- عقد HTML التفاعلي: بلا شبكة خارجية، وبسياسة محتوى محدّدة.
    ...
    -- إعادة النشر تستبدل الجسم بدل أن تسقط: تصحيح مكوّن عملٌ يومي.
  ...
  ELSIF _capability IN ('officialBookQuestions', 'selfTest') THEN
    -- رموز الأسئلة تُشتقّ من الحمولة المتحقَّق منها، لا من إعادة قراءة الجدول.
    ...
    LOOP
      ...
      -- نسخة منشورة واحدة فقط لكل سؤال؛ ما سواها يُصبح متجاوَزًا.
    ...
    -- «اختبر فهمك» يحتاج عضوية التقييم؛ وأسئلة الكتاب لا تدخله.
  ...
  -- draft_hash تُمسح في نفس التحديث: مشغّل cf10_block_ready_before_html_publication
  -- يرفض READY للخريطة والتجربة ما دامت مسودّتها غير مستهلَكة.
...
BEGIN
  ...
  -- المسار البسيط: لا مرحلة وسيطة ولا فصل مسؤوليات
  ...
  -- ولا يقرأ ولا يدّعي شيئًا عن المكوّنات الأخرى
...
-- Rollback: DROP FUNCTION public.golden_lesson_publish_component(uuid, text, text);
-- CF11 is untouched and remains the whole-lesson path.
